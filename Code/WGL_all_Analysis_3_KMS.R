...
MitoTranslationTranscription <- c("TRMT10C","HSD17B10","PRORP", 
                                  ...
                                  "MRPS28", "DAP3", "MRPS31", "MRPS33", "MRPS34", "MRPS35", "CHCHD1", "AURKAIP1", "PTCD3", "MRPS18B", # group 646
...
MitoElectronChainnAssembly <- c("NDUFS1","NDUFS2","NDUFS3","NDUFS7","NDUFS8","NDUFV1","NDUFV2", "NDUFAB1","NDUFA1",
                                ...
                                "UQCC1","UQCC2","UQCC3", "UQCC4","UQCC5","UQCC6", #group 645
...
MitoElectronChainnAssembly <- c("NDUFS1","NDUFS2","NDUFS3","NDUFS7","NDUFS8","NDUFV1","NDUFV2", "NDUFAB1","NDUFA1",
                                "NDUFA2","NDUFA3","NDUFA5","NDUFA6","NDUFA7","NDUFA8","NDUFA9","NDUFA10","NDUFA11","NDUFA12",
                                "NDUFA13","NDUFB1","NDUFB2","NDUFB3","NDUFB4","NDUFB5","NDUFB6","NDUFB7","NDUFB8","NDUFB9","NDUFB10",
                                "NDUFB11","NDUFC1","NDUFC2","NDUFS4","NDUFS5","NDUFS6","NDUFV3", # group 640 Mitochondrial complex I: NADH:ubiquinone oxidoreductase subunits
                                
                                "SDHA", "SDHB", "SDHC", "SDHD",  # group 641: Mitochondrial complex II: succinate dehydrogenase subunits (SDH)
                                
                                "UQCRB","UQCRQ","UQCRC1","UQCRC2","MT-CYB","CYC1","UQCRFS1","UQCRH","UQCR10","UQCR11", # group 642: Mitochondrial complex III: ubiquinol-cytochrome c reductase complex subunits (UQCR)
                                
                                "COX4I1","COX4I2","COX5A","COX5B","COX6A1", "COX6A2","COX6B1","COX6B2","COX6C", "COX7A1","COX7A2","COX7B","COX7B2","COX7C",
                                "COX8A","COX8C","MT-CO1","MT-CO2","MT-CO3", # group 643: Mitochondrial complex IV: cytochrome c oxidase subunits (COX, MT-CO)
                                
                                "ATP5F1A", "ATP5F1B", "ATP5F1C",  "ATP5F1D", "ATP5F1E", "ATP5MC1", "ATP5MC2", "ATP5MC3", "ATP5ME", "ATP5MF", "ATP5MG", "ATP5MJ", "ATP5MK", 
                                "MT-ATP6", "MT-ATP8", "ATP5PB", "ATP5PD", "ATP5PF", "ATP5PO", "ATP5IF1", # group 644: Mitochondrial complex V: ATP synthase subunits (ATP5, MT-ATP)
                                ...
                                "OCIAD2", "OXA1L", "PET100","PET117","PNKD", "SCO1", "SCO2",  "SDHAF1","SDHAF2","SDHAF3", "SDHAF4", "SFXN4",
                                "SMIM20", "SURF1","TACO1", "TIMMDC1","TMEM70","TMEM126A", "TMEM177","TMEM186","TMEM223","TMEM242","TTC19", 
                                "UQCC1","UQCC2","UQCC3", "UQCC4","UQCC5","UQCC6", # group 645 Mitochondrial respiratory chain complex assembly factors
                                
                                "ACAD9","COA1","ECSIT","NDUFAF1","TMEM126B","TMEM186", # group 1387: Mitochondrial complex I assembly complex
                                "BOLA3", "FDXR", "FXN", "GLRX5", "IBA57", "ISCA1", "ISCA2", "ISCU", "LYRM4", "NFS1", "NFU1", "NUBPL")#group 1853: Mitochondrial iron-sulfur assembly components    
# HGNC groups: 640,641,642,643,644, 645,1387,1853
...
# Save datasets
setwd(ResultsFile)
#write.csv(WGL.ALL.QN, "CRISPRScreens_BetaScores_WGL_QuantileNorm_Jun2026.csv")
#WGL.ALL.QN<- read.csv("CRISPRScreens_BetaScores_WGL_QuantileNorm_Jun2026.csv")
...
ggplot(WGL.ALL.QN.pvalue, aes(y=-log(An.minus.Eu.P,2), x=meanDiff))+
  ...
  geom_point(data= subset(WGL.ALL.QN.pvalue, An.minus.Eu.P<0.05 & meanDiff< -0.2 & MeanBeta <0 ), color = "red")+
  #geom_point(data= subset(WGL.ALL.QN.pvalue, Gene %in% c("HMGCS1") ), color = "blue")+
  theme_classic()+
...
ggplot(WGL.ALL.QN.pvalue, aes(y=MeanBeta, x=meanDiff)) +
  geom_point()+
...
# 5x4
# plot.BetaDelta.MeanBeta.WGL_QN.pdf
...
HighlightTheseGenes<- c("NDUFV1", "NDUFA1", "WAPAL")
HighlightTheseGenes<- subset(WGL.ALL.QN.pvalue, meanDiff < -0.2 & An.minus.Eu.P<0.05)$Gene
...
ggplot(WGL.ALL.QN.pvalue, aes(y=-log(An.minus.Eu.P,2), x=meanDiff))+
  ...
  geom_point(data = subset(WGL.ALL.QN.pvalue, Gene %in% HighlightTheseGenes ), color="Red") +
...
#GSEA.WGL<- read.csv("WGL.GSEA.csv")
# Dataset e
...
g.WGL.Aneu.Toxic<- gost(
  subset(WGL.ALL.QN.pvalue, meanDiff < -0.2 & MeanBeta<0)$Gene,
  organism = "hsapiens",
  ordered_query = FALSE,
  multi_query = FALSE,
  significant = TRUE,
...
setwd(ResultsFile)
#write.csv(g.WGL.Aneu.Toxic_result[,1:13], "gProfiler.WGL.AneuploidyToxic.Sheltzer_3.csv")
...
P.AnTox<-P.AnTox[c(1,3, 2,5,6,8,9,16, 4,15,21,22, 20,49,129),]
...
ggplot(P.AnTox, 
  ...
  coord_flip()
## 
# plot.gProfiler.sheltzer.WGL_5.pdf
# Figure 2C
# 10x5
...
# write.csv(g.WGL.Aneu.Toxic_result_fixed, "gProfiler.Sheltzer.WGL_AneuploidPathways.csv", row.names = FALSE)
    ...
    #### Gene groups per cell pair: #####
WGL.ALL.QN.pvalue

GeneGroups<- list(Proteasome<-c(Proteosome_Genes$Approved.symbol), 
                        Spliceosome<-c(RNA_Processing_Spliceosome), 
                        MitoETC<- c(MitoElectronChainnAssembly), 
                        MitoTT<- c(MitoTranslationTranscription), 
                        Ribosome<-c(Ribosomal_Genes_noMT$Approved.symbol), 
                        RNAProcessing<- c(RNA_Processing_rRNA)) 
GroupNames<- c("Proteasome", "Spliceosome", "Mitochondrial ETC", "Mitochondrial Transcription Translation", "Ribosome", "RNA Processing")

Group.per.Pair<- data.frame(Cell_Line= character(), 
                            GeneGroup= character(), 
                            MeanDiff= as.numeric() )

for (i in 1:6){
  DataGroup<- subset(WGL.ALL.QN.pvalue, Gene %in% GeneGroups[[i]])
  DLD1.13<- mean(DataGroup$DLD1.Ts13.Beta - DataGroup$DLD1.Cont.Beta)
  Vaco.13<- mean(DataGroup$Vaco432.Ts13.Beta - DataGroup$Vaco432.Cont.Beta)
  A2780.1q<- mean(DataGroup$A2780.Cont.Beta - DataGroup$A2780.Di1q.Beta)
  AGS.1q<-   mean(DataGroup$AGS.Cont.Beta - DataGroup$AGS.Di1q.Beta)
  MCF10A.1q<- mean(DataGroup$MCF10A.Cont.Beta - DataGroup$MCF10A.Di1q.Beta)
  A2058.1q<- mean(DataGroup$A2058.Cont.Beta - DataGroup$A2058.Di1q.Beta)
  A2058.7p<- mean(DataGroup$A2058.Cont.Beta - DataGroup$A2058.7pDi.Beta)
  Group.per.Pair<- rbind(Group.per.Pair, data.frame(Cell_Line= c("DLD1_Ts13", "Vaco431_Ts13", "A2780_1q", "AGS_1q", "MCF10A_1q", "A2058_1q", "A2058_7p"), 
                                                    GeneGroup= c(GroupNames[i],GroupNames[i],GroupNames[i],GroupNames[i],GroupNames[i],GroupNames[i],GroupNames[i]), 
                                                    MeanDiff= c(DLD1.13, Vaco.13, A2780.1q, AGS.1q, MCF10A.1q, A2058.1q, A2058.7p)
  ))
}
Group.per.Pair$GeneGroup<- factor(Group.per.Pair$GeneGroup, levels= c("Proteasome", "Spliceosome", "Mitochondrial ETC", "Mitochondrial Transcription Translation", "Ribosome", "RNA Processing"))
Group.per.Pair$Cell_Line<- factor(Group.per.Pair$Cell_Line, levels= c("DLD1_Ts13", "Vaco431_Ts13", "A2780_1q", "AGS_1q", "MCF10A_1q", "A2058_1q", "A2058_7p"))
Group.per.Pair$Cutoff<- Group.per.Pair$MeanDiff < -0.2

ggplot(Group.per.Pair, aes(x=Cell_Line, y=GeneGroup, fill=MeanDiff))+
  geom_tile()+
  ylab(" ")+
  xlab(" ")+
...
# set max aneuploidy score to 10? 
# very few values above 10 that disproportionately skew results. also >10 aneuploidy score might not be biologically different amount of aneuploidy stress. A lot of aneuploid stress. 
# Did not end up using "max 10" aneuploidy score. but might be worth revisiting in future analysis
#CRISPR_Gene_AneuScore$Score.divPloidyMax10 <- ifelse(CRISPR_Gene_AneuScore$Score.divPloidy > 10, 10, CRISPR_Gene_AneuScore$Score.divPloidy) 
...
ggplot(CRISPR_Gene_AneuScore, aes(y= UBE2H..7328., 
                                  x=Score.divPloidy))+
  geom_point()+
  theme_classic()+
  geom_hline(yintercept =0)+
...
cor.test(CRISPR_Gene_AneuScore$UBE2H..7328., CRISPR_Gene_AneuScore$Score.divPloidy, method = "pearson")
# cor =  -0.1773222
# p-value= 4.579e-06
...
MergeX2<- merge(x=CRISPR_DepMap_AneuScore[,1:9], y=AntiJoinDepMapCRISPR, by.x="Gene", by.y="Gene2")   # 28 extra genes merged
...
# Now merge merged DepMap-SchukkenWGL datasets: 
...
# Write this dataset! 
setwd("/Volumes/Schukken_SSD/CRISPR Screen")
...
# 5x4
# Plot.CRISPR.Depmap_MeanBeta.Coeff.diff_Mito.ETC.pdf
...
ggplot(Depmap.Sheltzer.WGL.Diff, aes(y=Aneu.divPloidy.CRISPR.Corr.Coef, x=meanDiff))+
  ...
  geom_point(data = subset(Depmap.Sheltzer.WGL.Diff, # MeanBeta> -1.5 & MeanBeta< -0.1 &
                           meanDiff < -0.2  & Aneu.divPloidy.CRISPR.Corr.Coef < -0.1 & 
                             Aneu.divPloidy.CRISPR.Corr.P< 0.005 & An.minus.Eu.P <0.1 & 
                             MeanBeta < 0), color = "red4") + 
  geom_point(data = subset(Depmap.Sheltzer.WGL.Diff, # MeanBeta> -1.5 & MeanBeta< -0.1 &
                           meanDiff < -0.2  & Aneu.divPloidy.CRISPR.Corr.Coef < -0.1 & 
                             Aneu.divPloidy.CRISPR.Corr.P< 0.005 & An.minus.Eu.P <0.05 & 
...
# 4x4
# Plot.CRISPR.DepmapCor.ploidy_vs_SheltzerBetaDelta.WGL_TopHits_MeanBeta.pdf
# Supplementary Figure 4A


# Top Hits: 
subset(Depmap.Sheltzer.WGL.Diff, # MeanBeta> -1.5 & MeanBeta< -0.1 &
       meanDiff < -0.2  & Aneu.divPloidy.CRISPR.Corr.Coef < -0.1 & 
         Aneu.divPloidy.CRISPR.Corr.P< 0.005 & An.minus.Eu.P <0.05 & 
         MeanBeta < 0)$Gene
subset(Depmap.Sheltzer.WGL.Diff, # MeanBeta> -1.5 & MeanBeta< -0.1 &
...
# mean difference in Beta score for genes on chrm 13: 
MeanTs13Diff<- mean(subset(WGL.ALL.QN.Ts13, Gene%in% GeneList13)$meanDiff) - mean(subset(WGL.ALL.QN.Ts13, ! Gene%in% GeneList13)$meanDiff)

# plot Ts13 corrected 
WGL.ALL.QN.Ts13$Ts13CorrectedDiff<- WGL.ALL.QN.Ts13$meanDiff
for(i in 1: length(WGL.ALL.QN.Ts13$Gene)){
  if (WGL.ALL.QN.Ts13$Gene[i] %in% GeneList13){
    WGL.ALL.QN.Ts13$Ts13CorrectedDiff[i]<- WGL.ALL.QN.Ts13$meanDiff[i] - MeanTs13Diff
  }
}


## Plot difference v average Beta (scatter plot)
WGL_TopHits<- subset(WGL.ALL.QN.Ts13, Ts13CorrectedDiff < -0.4 & MeanBeta < -0.2)$Gene
ggplot(WGL.ALL.QN.Ts13, aes(y=MeanBeta, x=Ts13CorrectedDiff, 
                            color= Ts13CorrectedDiff< -0.4 & MeanBeta < -0.2))+
  geom_point()+
  geom_density_2d(color = "white", bins = 10)+
...
mean(subset(WGL.ALL.QN.Ts13, ! Gene%in% GeneList13)$meanDiff) # - 0.0009065954
mean(subset(WGL.ALL.QN.Ts13,  Gene%in% GeneList13)$meanDiff) # - 0.06619442
mean(subset(WGL.ALL.QN.Ts13, ! Gene%in% GeneList13)$meanDiff) - mean(subset(WGL.ALL.QN.Ts13, Gene%in% GeneList13)$meanDiff) # - 0.06619442
# Genes on chrm 13 have mean Beta -0.06528783 more than genes not on Chrm 13. 



ggplot(subset(WGL.ALL.QN.Ts13, ! Gene%in% GeneList13), aes(x=Ts13CorrectedDiff))+
  geom_density()+
  geom_density(data = subset(WGL.ALL.QN.Ts13, Gene%in% GeneList13), color = "Red")+
  geom_hline(yintercept=0)+
  geom_vline(xintercept=0)+
  ...
  theme_classic()
# 5x4
# plot.Density.Ts13_corrected13.Screen.pdf


# more toxic to trisomy 
subset(WGL.ALL.QN.Ts13, Ts13CorrectedDiff< -0.6 & MeanBeta < -0.5)$Gene
...
HighlightTheseGenes<- subset(WGL.ALL.QN.Ts13, Ts13CorrectedDiff < -0.4)$Gene
...
ggplot(WGL.ALL.QN.Ts13, aes(x=Ts13CorrectedDiff))+
  ...
  ylab("Density")
# 4x4
# plot.WGL.Ts13.Density_Pathways_13corrected.pdf
# Supplementary Figure S5C

t.test(subset(WGL.ALL.QN.Ts13, !Gene %in% Proteosome_Genes$Approved.symbol)$Ts13CorrectedDiff, 
       subset(WGL.ALL.QN.Ts13, Gene %in% Proteosome_Genes$Approved.symbol)$Ts13CorrectedDiff) # 2.137e-07, corrected: 1.914e-07
t.test(subset(WGL.ALL.QN.Ts13, !Gene %in% RNA_Processing_Spliceosome)$Ts13CorrectedDiff, 
       subset(WGL.ALL.QN.Ts13, Gene %in% RNA_Processing_Spliceosome)$Ts13CorrectedDiff) # 9.114e-11, corrected: 7.848e-11
t.test(subset(WGL.ALL.QN.Ts13, !Gene %in% MitoElectronChainnAssembly)$Ts13CorrectedDiff, 
       subset(WGL.ALL.QN.Ts13, Gene %in% MitoElectronChainnAssembly)$Ts13CorrectedDiff) # 0.0002499643, corrected: 0.0001981
t.test(subset(WGL.ALL.QN.Ts13, !Gene %in% MitoTranslationTranscription)$Ts13CorrectedDiff, 
       subset(WGL.ALL.QN.Ts13, Gene %in% MitoTranslationTranscription)$Ts13CorrectedDiff) # 3.652714e-08, corrected: 5.847e-08
t.test(subset(WGL.ALL.QN.Ts13, !Gene %in% Ribosomal_Genes_noMT$Approved.symbol)$Ts13CorrectedDiff, 
       subset(WGL.ALL.QN.Ts13, Gene %in% Ribosomal_Genes_noMT$Approved.symbol)$Ts13CorrectedDiff) # 0.0001354, corrected: 0.0001316
t.test(subset(WGL.ALL.QN.Ts13, !Gene %in% RNA_Processing_rRNA)$Ts13CorrectedDiff, 
       subset(WGL.ALL.QN.Ts13, Gene %in% RNA_Processing_rRNA)$Ts13CorrectedDiff) # 2.544e-06, corrected: 2.776e-06




subset(WGL.ALL.QN.Ts13, Gene %in% GeneList13 & Ts13CorrectedDiff< -0.4)
# "INTS6" "PCID2" "CUL4A" 
...
ggplot(subset(CRISPR_DepMap_Ts13, ! Gene%in% GeneList13), aes(x=Gain.min.LossNeutral.Diff))+
  geom_density()+
  geom_density(data = subset(CRISPR_DepMap_Ts13, Gene%in% GeneList13), color = "Red")+
  geom_hline(yintercept=0)+
  geom_vline(xintercept=0)+
  scale_color_manual(values=c("Black", "Red"), name="Highlight")+ 
  theme_classic()
# 5x4
# plot.Density.Ts13.Screen.pdf
mean(subset(CRISPR_DepMap_Ts13, ! Gene%in% GeneList13)$Gain.min.LossNeutral.Diff) # 0.001510268
mean(subset(CRISPR_DepMap_Ts13,  Gene%in% GeneList13)$Gain.min.LossNeutral.Diff) # 0.002332794
mean(subset(CRISPR_DepMap_Ts13, ! Gene%in% GeneList13)$Gain.min.LossNeutral.Diff) - mean(subset(CRISPR_DepMap_Ts13, Gene%in% GeneList13)$Gain.min.LossNeutral.Diff) # - 0.0008225259
# Genes on chrm 13 have mean Beta 0.0008225259 higher than genes not on Chrm 13. 
# Not a significant lower beta score
# No need to correct. 
...
ggplot(DepMap.Sheltzer.WGL.CRISPR_Ts13, aes(x=Ts13CorrectedDiff, y=Gain.min.LossNeutral.Diff, 
                                            color= MeanBeta < 0 & Ts13CorrectedDiff < -0.2 & Gain.min.LossNeutral.Diff < -0.1))+
  geom_point()+
  geom_point(data = subset(DepMap.Sheltzer.WGL.CRISPR_Ts13, Ts13CorrectedDiff < -0.2 & 
                             Gain.min.LossNeutral.Diff < -0.1 & MeanBeta <0), color = "Red")+
  geom_hline(yintercept = -0.1, color="grey50", linetype = "dashed")+
  geom_vline(xintercept =-0.2, color="grey50", linetype = "dashed")+
  #geom_point(data = subset(DepMap.Sheltzer.WGL.CRISPR_Ts13, Gene %in% c("IRS2", "CTNNB1", "RAB10", "HMGCS1")), color = "Blue")+
  theme_classic()+
  geom_hline(yintercept = 0) +
  geom_vline(xintercept = 0) +
  geom_density_2d(color = "white", bins=10)+
...
# 5x4 
# plot.DepMap.Sheltzer.WGL.CRISPR_Ts13_MeanBeta0.pdf 

subset(DepMap.Sheltzer.WGL.CRISPR_Ts13, Ts13CorrectedDiff < -0.2 & Gain.min.LossNeutral.Diff < -0.1)$Gene
#"ACLY"     "ADSL"     "AP2S1"    "ATP6AP1"  "ATP6V0C"  "ATP6V0D1" "CFL1"     "CTNNB1"   "CYCS"     "DNM2"     "DUT"      "EFR3A"    "GINS2"    "HSPA8"   
#"MASTL"    "MYBL2"    "NMT1"     "NRBP1"    "NSF"      "RAB10"    "SAMD4B"   "SOX9"     "SUMO2"    "TCF7L2"   "TTC7A"    "TUBG1" 
...
ggplot(DepMap.Sheltzer.WGL.CRISPR_Ts13, aes(y=Gain.min.LossNeutral.Diff, x= Ts13CorrectedDiff,  
                                            color= Gene %in% HighlightTheseGenes))+
  geom_point()+
...
# write.csv(DepMap.Sheltzer.WGL.CRISPR_Ts13, "Chrm13_Specific_Toxicity_WGL.Sheltzer.DepMap.csv")
         ...
         ##### Compare WGL 13 Disomy to DepMap 13-Gain Specific hits: CRC only #####

CN13q.CRC<- Arm_CN[, c("X", "X13q")]
colnames(CN13q.CRC)<- c("Cell_Line", "13q")
CN13q.CRC<- subset(CN13q.CRC, Cell_Line %in% subset(Cell_Info, OncotreePrimaryDisease == "Colorectal Adenocarcinoma")$ModelID ) 
Cell_Neutral13q<- subset(CN13q.CRC, `13q`=="0")$Cell_Line
Cell_Gain13q<- subset(CN13q.CRC, `13q`=="1")$Cell_Line
Cell_Loss13q<- subset(CN13q.CRC, `13q`=="-1")$Cell_Line
Cell_Loss.Neutral13q<- subset(CN13q.CRC, `13q`!="1")$Cell_Line


CRISPR_DepMap_Ts13_CRC<-data.frame(GeneID= character(), 
                               Gain.LossNeutral.P = numeric(), 
                               Gain.min.LossNeutral.Diff= numeric()
)
...
for (i in 2:length(CRISPR_Gene)){
  ...
  if (length(na.omit(Drug_Ch13Gain[,2])) >5 & 
    ...
    CRISPR_DepMap_Ts13_CRC<- rbind(CRISPR_DepMap_Ts13_CRC, data.frame(GeneID= colnames(CRISPR_Gene)[i],
                                                              Gain.Neutral.P= test1$p.value, #p-value between drug scores with gain of 13 and drug score in cells neutral for 13
                                                              Gain.min.Neutral.Diff= test1$estimate[1]- test1$estimate[2], # Mean Drug score Gain13  minus  mean drugscore Neutral 13
                                                              Gain.LossNeutral.P = test2$p.value, 
                                                              Gain.min.LossNeutral.Diff= test2$estimate[1]- test2$estimate[2]) ) # Mean Drug score Gain13  minus  mean drugscore Neutral or loss 13
...
# length of Genes analyzed:  Genes
CRISPR_DepMap_Ts13_CRC<- CRISPR_DepMap_Ts13_CRC[order(CRISPR_DepMap_Ts13_CRC$Gain.Neutral.P),]
CRISPR_DepMap_Ts13_CRC[1:20,]


CRISPR_DepMap_Ts13_CRC$SigDiff<- "FALSE"
for (i in 1:length(CRISPR_DepMap_Ts13_CRC$GeneID)){
  if (CRISPR_DepMap_Ts13$Gain.LossNeutral.P[i]<0.05 && CRISPR_DepMap_Ts13$Gain.min.LossNeutral.Diff[i]< -0.1){
    CRISPR_DepMap_Ts13$SigDiff[i]<- "TRUE"
  }
}
subset(CRISPR_DepMap_Ts13_CRC, SigDiff=="TRUE")
CRISPR_DepMap_Ts13_CRC$Gene<- sub("[..].*", "", CRISPR_DepMap_Ts13_CRC$GeneID)



ggplot(CRISPR_DepMap_Ts13_CRC, aes(y=-log(Gain.LossNeutral.P,2), x=Gain.min.LossNeutral.Diff, 
                                   color= SigDiff))+
  geom_point()+
  geom_point(data = subset(CRISPR_DepMap_Ts13_CRC, SigDiff == TRUE), color = "Red")+
  theme_classic()+
  geom_density_2d(color = "white")+
  xlab("Difference in Gene Beta Score (DepMap): \nGain 13 minus Neutral/Loss 13")+
  ylab("-log2(P-value)")+
...
subset(CRISPR_DepMap_Ts13_CRC, Gain.LossNeutral.P < 0.05 & Gain.min.LossNeutral.Diff < -0.1)$Gene


subset(CRISPR_DepMap_Ts13_CRC, Gene == "IRS2")

# Things less toxic to Ts13 cells: (DepMap) 
subset(CRISPR_DepMap_Ts13_CRC, Gain.LossNeutral.P < 0.001 & Gain.min.LossNeutral.Diff > 0.08)$Gene




## Now merge my WGL. CRISPR screen data with Ts13q DepMap data ###
DepMap.Sheltzer.WGL.CRISPR_Ts13_CRC<- merge(WGL.ALL.QN.Ts13,  CRISPR_DepMap_Ts13_CRC, by.x="Gene", by.y="Gene") 

ggplot(DepMap.Sheltzer.WGL.CRISPR_Ts13_CRC, aes(x=Ts13CorrectedDiff, y=Gain.min.LossNeutral.Diff, 
                                            color= MeanBeta < 0 & Ts13CorrectedDiff < -0.2 & Gain.min.LossNeutral.Diff < -0.1))+
  geom_point()+
  geom_point(data = subset(DepMap.Sheltzer.WGL.CRISPR_Ts13_CRC, Ts13CorrectedDiff < -0.2 & 
                             Gain.min.LossNeutral.Diff < -0.1 & MeanBeta <0), color = "Red")+
  geom_hline(yintercept = -0.1, color="grey50", linetype = "dashed")+
  geom_vline(xintercept =-0.2, color="grey50", linetype = "dashed")+
  #geom_point(data = subset(DepMap.Sheltzer.WGL.CRISPR_Ts13_CRC, Gene %in% c("HMGCS1")), color = "Blue")+
  theme_classic()+
  geom_hline(yintercept = 0) +
  geom_vline(xintercept = 0) +
  geom_density_2d(color = "white", bins=10)+
  ylab("Difference in CRISPR Score (DepMap, CRC): \nGain 13 minus Neutral/Loss 13")+
  xlab("Difference in  Beta Score (Sheltzer): \nGain 13 minus Loss 13")+
  scale_color_manual(values= c("Black", "Red"), name="")
# 5x4 
# plot.DepMap.Sheltzer.WGL.CRISPR_Ts13_MeanBeta0_onlyCRC.pdf 
# Figure 4A Ts13

subset(DepMap.Sheltzer.WGL.CRISPR_Ts13_CRC, Ts13CorrectedDiff < -0.2 & 
         Gain.min.LossNeutral.Diff < -0.1 & MeanBeta <0)$Gene

# HMGCS1 top gene, cholestoral pathway. upregulated in CRC
...
### Look at genes located ON chromosome 13: 
Chrm13Genes<- unique(subset(Gene_Info, `Chromosome/scaffold name` == 13)$`HGNC symbol`)
subset(DepMap.Sheltzer.WGL.CRISPR_Ts13_CRC, Gene %in% Chrm13Genes)
...
ggplot(DepMap.Sheltzer.WGL.CRISPR_Ts13_CRC, aes(y=Gain.min.LossNeutral.Diff, x= Ts13CorrectedDiff,  
                                            color= Gene %in% HighlightTheseGenes))+
  geom_point()+
  geom_point(data = subset(DepMap.Sheltzer.WGL.CRISPR_Ts13_CRC, Gene %in% HighlightTheseGenes), color="red")+
  theme_classic()+
  geom_hline(yintercept = 0) +
  geom_vline(xintercept = 0) +
  xlab("Difference in Gene Beta Score (DepMap): \nGain 13 minus Neutral/Loss 13")+
  ...
  scale_color_manual(values= c("Black", "Red"), name="")
# 5x4 
# DepMap.Sheltzer.WGL.CRISPR_Ts13_CRC 
...
subset(DepMap.Sheltzer.WGL.CRISPR_Ts13_CRC, Gene %in% HighlightTheseGenes & Gain.min.LossNeutral.Diff < -0.1 )

setwd(ResultsFile)
#write.csv(DepMap.Sheltzer.WGL.CRISPR_Ts13_CRC, "Chrm13_Specific_Toxicity_WGL.Sheltzer.DepMapCRC.csv")
# DepMap.Sheltzer.WGL.CRISPR_Ts13_CRC<- read.csv( "Chrm13_Specific_Toxicity_WGL.Sheltzer.DepMapCRC.csv")




         ##### g:profiler Ts13- Specific: ####

# G-Profiler: 
g.WGL.Ts13<- gost(
  subset(WGL.ALL.QN.Ts13, Ts13CorrectedDiff < -0.2)$Gene,
  organism = "hsapiens",
  ordered_query = FALSE,
  multi_query = FALSE,
  significant = TRUE,
  ...
  correction_method = c("g_SCS"),
  domain_scope = "custom",
  custom_bg = as.character(WGL.ALL.QN.Ts13$Gene),
  numeric_ns = "",
  sources = NULL,
  as_short_link = FALSE
)

g.WGL.Ts13_result<-g.WGL.Ts13$result
g.WGL.Ts13_result<-g.WGL.Ts13_result[order(g.WGL.Ts13_result$p_value),]


##Toxic to aneuploid cells Sheltzer
P.Ts13<-g.WGL.Ts13_result[order(g.WGL.Ts13_result$p_value),]
P.Ts13<- subset(P.Ts13, !source %in% c("HPA", "TF"))
P.Ts13$term_name[1:100]
P.Ts13<-P.Ts13[c(1,2,8,  3,10,19,30,36,  12,29,73,  20,34,46,  31,32),]
P.Ts13$term_name<- factor(P.Ts13$term_name, levels= P.Ts13$term_name)
P.Ts13$Termtype<-c( "Protein complex","Protein complex","Protein complex",
...
ggplot(P.Ts13, 
  ...
  ylab("-log2 (P-Value)") +
  xlab("Key Biological terms")+
  ggtitle ("Biological terms enriched Ts13")+
  theme_classic()+
  coord_flip()
# plot.gProfiler.sheltzer.Ts13_Chrm13Corrected.pdf
...
# Write g profiler results
setwd(ResultsFile)
g.WGL.Ts13_result2 <- as.data.frame(lapply(g.WGL.Ts13_result, function(col) {
  if (is.list(col)) {
    sapply(col, function(x) paste(x, collapse = ", "))
  } else {
    col
...
# write.csv(g.WGL.Ts13_result2, "gProfiler.WGL.Ts13_chrm13corrected.csv", row.names = FALSE)
...
## Now calculate Beta score correcting for chromosome trisomy dependency: 
# mean difference in Beta score for genes on chrm 13: 
mean(subset(WGL.ALL.QN.7pDi, !Gene%in% GeneList7p)$meanDiff) # -0.0008193034
mean(subset(WGL.ALL.QN.7pDi, Gene%in% GeneList7p)$meanDiff)  # 0.05826246
MeanTs7pDiff<- mean(subset(WGL.ALL.QN.7pDi, Gene%in% GeneList7p)$meanDiff) - mean(subset(WGL.ALL.QN.7pDi, ! Gene%in% GeneList7p)$meanDiff)

WGL.ALL.QN.7pDi$Ts7pCorrectedDiff<- WGL.ALL.QN.7pDi$meanDiff
for(i in 1: length(WGL.ALL.QN.7pDi$Gene)){
  if (WGL.ALL.QN.7pDi$Gene[i] %in% GeneList7p){
    WGL.ALL.QN.7pDi$Ts7pCorrectedDiff[i]<- WGL.ALL.QN.7pDi$meanDiff[i] - MeanTs7pDiff
  }
}
...
WGL_TopHits<- subset(WGL.ALL.QN.7pDi, meanBeta< -0.2 & Ts7pCorrectedDiff < -0.4)$Gene
ggplot(WGL.ALL.QN.7pDi, aes(y=meanBeta, x=Ts7pCorrectedDiff, 
                            color=meanBeta < -0.2 & Ts7pCorrectedDiff< -0.4))+
  geom_point()+
  geom_density_2d(color = "white", bins = 10)+
...
ggplot(WGL.ALL.QN.7pDi, aes(x=Ts7pCorrectedDiff))+
...
t.test(subset(WGL.ALL.QN.7pDi, !Gene %in% Proteosome_Genes$Approved.symbol)$Ts7pCorrectedDiff, 
       subset(WGL.ALL.QN.7pDi, Gene %in% Proteosome_Genes$Approved.symbol)$Ts7pCorrectedDiff) # 0.00571 LESS DEPENDENT, corrected: 0.005568
t.test(subset(WGL.ALL.QN.7pDi, !Gene %in% RNA_Processing_Spliceosome)$Ts7pCorrectedDiff, 
       subset(WGL.ALL.QN.7pDi, Gene %in% RNA_Processing_Spliceosome)$Ts7pCorrectedDiff) # 0.01807 LESS DEPENDENT, corrected:0.01949

x<- t.test(subset(WGL.ALL.QN.7pDi, !Gene %in% MitoElectronChainnAssembly)$Ts7pCorrectedDiff, 
           subset(WGL.ALL.QN.7pDi, Gene %in% MitoElectronChainnAssembly)$Ts7pCorrectedDiff) # 6.16245e-17 ***, corrected: 5.669529e-17
x<- t.test(subset(WGL.ALL.QN.7pDi, !Gene %in% MitoTranslationTranscription)$Ts7pCorrectedDiff, 
           subset(WGL.ALL.QN.7pDi, Gene %in% MitoTranslationTranscription)$Ts7pCorrectedDiff) # 1.795465e-18 ***, corrected: 1.042379e-18

t.test(subset(WGL.ALL.QN.7pDi, !Gene %in% Ribosomal_Genes_noMT$Approved.symbol)$Ts7pCorrectedDiff, 
       subset(WGL.ALL.QN.7pDi, Gene %in% Ribosomal_Genes_noMT$Approved.symbol)$Ts7pCorrectedDiff) # 0.8745 NS, corrected:0.8508
t.test(subset(WGL.ALL.QN.7pDi, !Gene %in% RNA_Processing_rRNA)$Ts7pCorrectedDiff, 
       subset(WGL.ALL.QN.7pDi, Gene %in% RNA_Processing_rRNA)$Ts7pCorrectedDiff) # 0.4104 NS, corrected:0.401



# More toxic to trisomy 
subset(WGL.ALL.QN.7pDi, meanBeta< -0.2 & Ts7pCorrectedDiff < -1)$Gene
...
subset(WGL.ALL.QN.7pDi, meanBeta< 0 & Ts7pCorrectedDiff < -.4 & Gene %in% GeneList7p)$Gene
...
ggplot(DepMap.Sheltzer.WGL.CRISPR_Ts7p, aes(x=Ts7pCorrectedDiff, y=Gain.min.LossNeutral.Diff, 
                                            color= meanBeta<0 & Ts7pCorrectedDiff < -0.2 & Gain.min.LossNeutral.Diff < -0.1))+
  geom_point()+
  geom_point(data = subset(DepMap.Sheltzer.WGL.CRISPR_Ts7p, Ts7pCorrectedDiff < -0.2 & 
                             Gain.min.LossNeutral.Diff < -0.1 & meanBeta <0))+
  geom_hline(yintercept = -0.1, color="grey50", linetype = "dashed")+
  geom_vline(xintercept =-0.2, color="grey50", linetype = "dashed")+
  ...
  scale_color_manual(values= c("Black", "Red"), name="")
# 5x4 
# plot.DepMap.Sheltzer.WGL.CRISPR_Ts7p_7pcorrected.pdf 
# Figure 4A

subset(DepMap.Sheltzer.WGL.CRISPR_Ts7p, Gene == "GRB2")

# potential hits more essential to 7p trisomy cells: 
subset(DepMap.Sheltzer.WGL.CRISPR_Ts7p, Ts7pCorrectedDiff < -0.2 & Gain.min.LossNeutral.Diff < -0.1) #$Gene
...
# Depmap only chromosome 7 hits: ACTB (7p)  and SNRPB2 (7p)
         ...
         ##### g:profiler Ts7p- Specific: ####

# G-Profiler: 
g.WGL.Ts7p<- gost(
  subset(WGL.ALL.QN.7pDi, Ts7pCorrectedDiff < -0.2)$Gene,
  organism = "hsapiens",
  ordered_query = FALSE,
  multi_query = FALSE,
  significant = TRUE,
  ...
  correction_method = c("g_SCS"),
  domain_scope = "custom",
  custom_bg = as.character(WGL.ALL.QN.7pDi$Gene),
  numeric_ns = "",
  sources = NULL,
  as_short_link = FALSE
)

g.WGL.Ts7p_result<-g.WGL.Ts7p$result
g.WGL.Ts7p_result<-g.WGL.Ts7p_result[order(g.WGL.Ts7p_result$p_value),]


##Toxic to aneuploid cells Sheltzer
P.Ts7<-g.WGL.Ts7p_result[order(g.WGL.Ts7p_result$p_value),]
P.Ts7<- subset(P.Ts7, !source %in% c("HPA", "TF"))
P.Ts7$term_name
P.Ts7<-P.Ts7[c(1,2,7,9,13,17,32,35,  10,11,12, 117 ),]
P.Ts7$term_name<- factor(P.Ts7$term_name, levels= P.Ts7$term_name)
P.Ts7$Termtype<-c(  "Mitochondrial Metabolism","Mitochondrial Metabolism", "Mitochondrial Metabolism",
                    "Mitochondrial Metabolism","Mitochondrial Metabolism","Mitochondrial Metabolism",
                    "Mitochondrial Metabolism","Mitochondrial Metabolism",
                    "Ribosome","Ribosome","Ribosome",
                    "Protein complex"
...
ggplot(P.Ts7, 
  ...
  ylab("-log2 (P-Value)") +
  xlab("Key Biological terms")+
  ggtitle ("Biological terms enriched Ts7p")+
  theme_classic()+
  coord_flip()
# plot.gProfiler.sheltzer.Ts7p.pdf
...
# Write g profiler results
setwd(ResultsFile)
g.WGL.Ts7p_result2 <- as.data.frame(lapply(g.WGL.Ts7p_result, function(col) {
  if (is.list(col)) {
    sapply(col, function(x) paste(x, collapse = ", "))
  } else {
    col
...
#write.csv(g.WGL.Ts7p_result2, "gProfiler.WGL.Ts7p.csv", row.names = FALSE)
...
# plot 1q corrected 
## Now calculate Beta score correcting for chromosome trisomy dependency: 
# mean difference in Beta score for genes on chrm 1q: 
mean(subset(WGL.ALL.QN.1qDi, !Gene%in% GeneList1q)$meanDiff) # 0.001460659
mean(subset(WGL.ALL.QN.1qDi, Gene%in% GeneList1q)$meanDiff)  # -0.04917701
MeanTs1qDiff<- mean(subset(WGL.ALL.QN.1qDi, Gene%in% GeneList1q)$meanDiff) - mean(subset(WGL.ALL.QN.1qDi, ! Gene%in% GeneList13)$meanDiff)

WGL.ALL.QN.1qDi$Ts1qCorrectedDiff<- WGL.ALL.QN.1qDi$meanDiff
for(i in 1: length(WGL.ALL.QN.1qDi$Gene)){
  if (WGL.ALL.QN.1qDi$Gene[i] %in% GeneList1q){
    WGL.ALL.QN.1qDi$Ts1qCorrectedDiff[i] <- WGL.ALL.QN.1qDi$meanDiff[i] - MeanTs1qDiff
  }
}
...
WGL_TopHits<- subset(WGL.ALL.QN.1qDi, Ts1qCorrectedDiff< -0.25 & An.minus.Eu.P < 0.05)$Gene
ggplot(WGL.ALL.QN.1qDi, aes(y=-log2(An.minus.Eu.P), x=Ts1qCorrectedDiff, 
                            color= An.minus.Eu.P< 0.05 & Ts1qCorrectedDiff< -0.25))+
  geom_point()+
  geom_density_2d(color = "white", bins = 10)+
...
subset(WGL.ALL.QN.1qDi, meanBeta< -0.1 & Ts1qCorrectedDiff < -0.25 & An.minus.Eu.P < 0.05)$Gene


# write.csv(WGL.ALL.QN.1qDi, "WGL.QN.1qDi.csv")

# More toxic to trisomy 
subset(WGL.ALL.QN.1qDi, meanBeta< -0.1 & Ts1qCorrectedDiff < -0.5 & An.minus.Eu.P < 0.05)$Gene


subset(WGL.ALL.QN.1qDi, meanBeta< -0.1 & Ts1qCorrectedDiff < -0.25 & An.minus.Eu.P < 0.05 & Gene %in% GeneList1q)$Gene
...
ggplot(subset(WGL.ALL.QN.1qDi, !Gene %in% GeneList1q), aes(x=Ts1qCorrectedDiff)) +
  geom_density()+
...
ggplot(WGL.ALL.QN.1qDi, aes(x=Ts1qCorrectedDiff))+
...
t.test(subset(WGL.ALL.QN.1qDi, !Gene %in% Proteosome_Genes$Approved.symbol)$Ts1qCorrectedDiff, 
       subset(WGL.ALL.QN.1qDi, Gene %in% Proteosome_Genes$Approved.symbol)$Ts1qCorrectedDiff) # 0.04679, corrected: 0.04655
t.test(subset(WGL.ALL.QN.1qDi, !Gene %in% RNA_Processing_Spliceosome)$Ts1qCorrectedDiff, 
       subset(WGL.ALL.QN.1qDi, Gene %in% RNA_Processing_Spliceosome)$Ts1qCorrectedDiff) # 0.2411  NS , corrected:0.2147
t.test(subset(WGL.ALL.QN.1qDi, !Gene %in% MitoElectronChainnAssembly)$Ts1qCorrectedDiff, 
       subset(WGL.ALL.QN.1qDi, Gene %in% MitoElectronChainnAssembly)$Ts1qCorrectedDiff) # 7.625e-09, corrected:7.79e-09
t.test(subset(WGL.ALL.QN.1qDi, !Gene %in% MitoTranslationTranscription)$Ts1qCorrectedDiff, 
       subset(WGL.ALL.QN.1qDi, Gene %in% MitoTranslationTranscription)$Ts1qCorrectedDiff) # 2.889e-05, corrected:3.668e-05
t.test(subset(WGL.ALL.QN.1qDi, !Gene %in% Ribosomal_Genes_noMT$Approved.symbol)$Ts1qCorrectedDiff, 
       subset(WGL.ALL.QN.1qDi, Gene %in% Ribosomal_Genes_noMT$Approved.symbol)$Ts1qCorrectedDiff) # 0.04321, corrected:0.03572
t.test(subset(WGL.ALL.QN.1qDi, !Gene %in% RNA_Processing_rRNA)$Ts1qCorrectedDiff, 
       subset(WGL.ALL.QN.1qDi, Gene %in% RNA_Processing_rRNA)$Ts1qCorrectedDiff) # 0.0003232, corrected:0.0003068
...
ggplot(CRISPR_DepMap_Ts1q, aes(y=-log(Gain.LossNeutral.P,2), x=Gain.min.LossNeutral.Diff, 
                               color= SigDiff))+
  geom_point()+
...
ggplot(DepMap.Sheltzer.WGL.CRISPR_Ts1q, aes(x=meanDiff, y=Gain.min.LossNeutral.Diff))+
  ...
  geom_point(data = subset(DepMap.Sheltzer.WGL.CRISPR_Ts1q, 
                           meanDiff < -0.2 & Gain.min.LossNeutral.Diff < -0.1 & meanBeta < 0 ), 
             color="red")+
  ...
  scale_color_manual(values= c("Black", "Red"), name="")
# 5x4 
# plot.DepMap.Sheltzer.WGL.CRISPR_Ts1q_1qcorrected.pdf 
# Figure 4A
...
#write.csv(DepMap.Sheltzer.WGL.CRISPR_Ts1q, "Chrm1q_Specific_Toxicity_WGL.Sheltzer.DepMap.csv")
         ...
         ##### g:profiler Ts1q- Specific: ####

# G-Profiler: 
g.WGL.Ts1q<- gost(
  subset(WGL.ALL.QN.1qDi, Ts1qCorrectedDiff < -0.2)$Gene,
  organism = "hsapiens",
  ordered_query = FALSE,
  multi_query = FALSE,
  significant = TRUE,
  ...
  correction_method = c("g_SCS"),
  domain_scope = "custom",
  custom_bg = as.character(WGL.ALL.QN.1qDi$Gene),
  numeric_ns = "",
  sources = NULL,
  as_short_link = FALSE
)

g.WGL.Ts1q_result<-g.WGL.Ts1q$result
g.WGL.Ts1q_result<-g.WGL.Ts1q_result[order(g.WGL.Ts1q_result$p_value),]


##Toxic to aneuploid cells Sheltzer
P.Ts1<-g.WGL.Ts1q_result[order(g.WGL.Ts1q_result$p_value),]
P.Ts1<- subset(P.Ts1, !source %in% c("HPA", "TF"))
P.Ts1$term_name[1:100]
P.Ts1<-P.Ts1[c(1,2, 3,10,22, 4,6,43, 11,15,17,18,29, 13,28,51 ),]
P.Ts1$term_name<- factor(P.Ts1$term_name, levels= P.Ts1$term_name)
P.Ts1$Termtype<-c(  "Protein complex", "Protein complex", 
                    "Ribosome","Ribosome","Ribosome",
                    "RNA Processing","RNA Processing","RNA Processing",
                    "Mitochondrial Metabolism","Mitochondrial Metabolism", "Mitochondrial Metabolism",
                    "Mitochondrial Metabolism","Mitochondrial Metabolism",
...
ggplot(P.Ts1, 
  ...
  ylab("-log2 (P-Value)") +
  xlab("Key Biological terms")+
  ggtitle ("Biological terms enriched Ts1q")+
  theme_classic()+
  coord_flip()
# plot.gProfiler.sheltzer.Ts1q.pdf
...
# Write g profiler results
setwd(ResultsFile)
g.WGL.Ts1q_result2 <- as.data.frame(lapply(g.WGL.Ts1q_result, function(col) {
  if (is.list(col)) {
    sapply(col, function(x) paste(x, collapse = ", "))
  } else {
    col
...
#write.csv(g.WGL.Ts1q_result2, "gProfiler.WGL.Ts1q.csv", row.names = FALSE)
...
ggplot(Merge.WGL.QN.Corr, aes(AneuScore.WGL.Corr, MeanBeta)) + 
  ...
  geom_vline(xintercept = 0)+
  geom_point()+
  geom_point(data = subset(Merge.WGL.QN.Corr, AneuScore.WGL.Corr < -.2 & AneuScore.Corr.P < 0.05 & MeanBeta < 0), color = "red") + 
  ...
  #geom_point(data = subset(Merge.WGL.QN.Corr, Gene == "FOSL1"), color = "Blue") + 
  geom_density_2d(color = "white", bins = 10)+
  ylab("Mean Beta Score")+
...
# CTLH complex: "HMGCS1", "HMGCS2", "WDR26", "MKNL1", "MAEA", "GID4", "ARMC8", "RMND5A", "TWA1", "RANBP9", "GID8", "UBE2H"
...
ggplot(WGL.categoricalDiff.Correlation, aes(x=AneuScore.WGL.Corr, y=meanDiff))+
  geom_point()+
  #geom_point(data = subset(WGL.categoricalDiff.Correlation, 
  #                         AneuScore.WGL.Corr < -0.1 & 
  #                           meanDiff < -0.2 & # average difference in beta score. 
  #                           MeanBeta.x <0), # MeanBeta.x is the average beta score in paire WGL analysis
  #           color = "red")+
  geom_point(data = subset(WGL.categoricalDiff.Correlation, Gene %in% Proteosome_Genes$Approved.symbol), color = "chartreuse1") + 
  geom_point(data = subset(WGL.categoricalDiff.Correlation, Gene %in% RNA_Processing_Spliceosome), color = "deepskyblue1")+
  ...
  geom_point(data = subset(WGL.categoricalDiff.Correlation, Gene %in% Ribosomal_Genes_noMT$Approved.symbol), color = "#C77CFF")+
  geom_point(data = subset(WGL.categoricalDiff.Correlation, Gene %in% RNA_Processing_rRNA), color = "deepskyblue4")+
  #geom_point(data = subset(WGL.categoricalDiff.Correlation, Gene %in% c("HMGCS1", "WDR26", "UBE2H")), color = "Red") + 
  theme_classic()+
  geom_density_2d(color = "white")+ 
  geom_hline(yintercept = 0)+
  geom_vline(xintercept = 0)+
  xlab("Total aneuploidy burden correlation (WGL): \n Aneuploidy Score  & Beta Score")+
  ylab("Difference in Beta Score (WGL): \n Aneuploid - Euploid")+
  scale_color_manual(values= c("Black", "Red"), name="Top hits")
# 5x4
# plot.Sheltzer.WGL_AneuScoreCorr_betaDelta.pdf
...
ggplot(Merge.WGL.QN.Corr, aes(x=AneuScore.WGL.Corr))+
  geom_vline(xintercept=0, color="grey50")+
  geom_hline(yintercept=0, color="grey50")+
  geom_density(data = subset(Merge.WGL.QN.Corr, ! Gene %in% c(Proteosome_Genes$Approved.symbol, RNA_Processing_Spliceosome, MitoElectronChainnAssembly, MitoTranslationTranscription, Ribosomal_Genes_noMT$Approved.symbol, RNA_Processing_rRNA)), color = "black" ) +
  geom_density(data = subset(Merge.WGL.QN.Corr, Gene %in% Proteosome_Genes$Approved.symbol), color = "chartreuse1") + 
  geom_density(data = subset(Merge.WGL.QN.Corr, Gene %in% RNA_Processing_Spliceosome), color = "deepskyblue1")+
  geom_density(data = subset(Merge.WGL.QN.Corr, Gene %in% MitoElectronChainnAssembly), color = "#F8766D") + 
  geom_density(data = subset(Merge.WGL.QN.Corr, Gene %in% MitoTranslationTranscription), color = "gold2") + 
  geom_density(data = subset(Merge.WGL.QN.Corr, Gene %in% Ribosomal_Genes_noMT$Approved.symbol), color = "#C77CFF")+
  geom_density(data = subset(Merge.WGL.QN.Corr, Gene %in% RNA_Processing_rRNA), color = "deepskyblue4")+
  theme_classic()+
  xlim(-1,1)+
  xlab("Correlation: Beta Score & Aneuploidy Score\n<- Aneuploid dependency")+
  ylab("Density")
# 5x4
# plot.WGL.AneuScoreCorrelation_Pathways.pdf
controlCorr<-subset(Merge.WGL.QN.Corr, ! Gene %in% c(Proteosome_Genes$Approved.symbol, RNA_Processing_Spliceosome, MitoElectronChainnAssembly, MitoTranslationTranscription, Ribosomal_Genes_noMT$Approved.symbol, RNA_Processing_rRNA))$AneuScore.WGL.Corr
t.test(controlCorr, subset(Merge.WGL.QN.Corr, Gene %in% Proteosome_Genes$Approved.symbol)$AneuScore.WGL.Corr) # 0.04564
t.test(controlCorr, subset(Merge.WGL.QN.Corr, Gene %in% RNA_Processing_Spliceosome)$AneuScore.WGL.Corr) #1.525e-11
t.test(controlCorr, subset(Merge.WGL.QN.Corr, Gene %in% MitoElectronChainnAssembly)$AneuScore.WGL.Corr) #2.69e-11
t.test(controlCorr, subset(Merge.WGL.QN.Corr, Gene %in% MitoTranslationTranscription)$AneuScore.WGL.Corr) #1.206e-13
t.test(controlCorr, subset(Merge.WGL.QN.Corr, Gene %in% Ribosomal_Genes_noMT$Approved.symbol)$AneuScore.WGL.Corr) # 0.0001387
t.test(controlCorr, subset(Merge.WGL.QN.Corr, Gene %in% RNA_Processing_rRNA)$AneuScore.WGL.Corr) #0.03645
...
# Double check: only look at DepMap data with >200 samples
DepMap.Sheltzer.WGL_AneuScore<- subset(DepMap.Sheltzer.WGL_AneuScore, GeneID %in% subset(CRISPR_DepMap_AneuScore, SampleNum > 200)$GeneID)
...
ggplot(DepMap.Sheltzer.WGL_AneuScore, aes(x=AneuScore.WGL.Corr, y=Aneu.divPloidy.CRISPR.Corr.Coef))+
  ...
  geom_point(data = subset(DepMap.Sheltzer.WGL_AneuScore, # MeanBeta> -1.5 & MeanBeta< -0.1 &
                           AneuScore.WGL.Corr < -0.1  & Aneu.divPloidy.CRISPR.Corr.Coef < -0.1 & 
                             ...
                             MeanBeta <0), color = "red") + 
  #geom_point(data = subset(DepMap.Sheltzer.WGL_AneuScore, Gene %in% c("PPP1R12A", "UBE2H", "WRD26", "FOXM1")), color = "Blue") + # use this to identify a gene of interest in blue
  theme_classic()+
  geom_density_2d(color = "white", bins=10)+ 
  geom_hline(yintercept = 0)+
  geom_vline(xintercept = 0)+
  ...
  scale_color_manual(values= c("Black", "Red"), name="Top hits")
# 5x4
# plot.DepMap.Sheltzer.WGL_AneuScore_AllAneudivPloidy_MeanBeta0.pdf
...
ggplot(DepMap.Sheltzer.WGL_AneuScore, aes(x=AneuScore.WGL.Corr, y=Aneu.divPloidy.CRISPR.Corr.Coef))+
  ...
  ylab("Correlation (DepMap): \n Aneuploidy Score correlation with Effect Score")
# 5x4
# plot.DepMap.Sheltzer.WGL_AneuScore_AllAneudivPloidy_Pathway.pdf
# Supplementary Figure S4D



# When you look at the subset of WGL data that overlaps with DepMap, you get these p-values: 
# I used the p-values corresponding to all WGL data. a few more genes.
...
# now get rid of the correlation between aneuploidy.score and aneuploidy.score. (100% correlation, coefficient ==1)
CRISPR.DepMap_AneuScore_lm_Lineage <- subset(CRISPR.DepMap_AneuScore_lm_Lineage, AneuDivPloidy.CRISPR.lm.Coef< 1)
...
ggplot(CRISPR.DepMap_AneuScore_lm_Lineage, aes(x=AneuDivPloidy.CRISPR.lm.Coef, 
  ...
  ylab(paste("-log2(p-value)"))+
  ggtitle(paste("DepMap Linear model: tissue type & aneuploidy score"))
# plot.LM.DepMap_AndivPloidy_pathway2_all.pdf
# 4x4
