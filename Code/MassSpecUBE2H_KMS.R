...
ggplot(PlotUBE2H, aes(x=Sample, 
                y= Abundance, fill=Sample ))+
  geom_boxplot()+
  geom_point()+
  scale_fill_manual(values= c("turquoise2","turquoise2","mediumorchid2","mediumorchid2"))+
...
ggplot(MS2, aes(x=Abundance.Ratio..log2....UBE2HKO.....WT., 
  ...
  geom_point()+ 
  theme_classic()+
  #geom_point(data = subset(MS2, Gene.Symbol...1 %in% c("HMGCS1", "HMGCS2", "NMNAT1", "PALLD", "MKLN1", "VASP")), color = "red") + 
...
MS2_UBE_Down <- quantile(MS2$Abundance.Ratio..log2....UBE2HKO.....WT., 0.20, na.rm = TRUE) #bottom 20% of protein <= this value
MS2_UBE_Up <- quantile(MS2$Abundance.Ratio..log2....UBE2HKO.....WT., 0.80, na.rm = TRUE) #Top 20% of protein >= this value

MS2_AZ_Down <- quantile(MS2$Abundance.Ratio..log2....AZ.....WT., 0.20, na.rm = TRUE) #bottom 20 of protein <= this value
MS2_AZ_Up <- quantile(MS2$Abundance.Ratio..log2....AZ.....WT., 0.80, na.rm = TRUE) #Top 20 of protein >= this value

MS2_UBEAZ.AZ_Down <- quantile(MS2$Abundance.Ratio..log2....UBE2HKO_AZ.....AZ., 0.20, na.rm = TRUE) #bottom 20 of protein <= this value
MS2_UBEAZ.AZ_Up <- quantile(MS2$Abundance.Ratio..log2....UBE2HKO_AZ.....AZ., 0.80, na.rm = TRUE) #Top 20 of protein >= this value

MS2_UBEAZ.UBE_Down <- quantile(MS2$Abundance.Ratio..log2....UBE2HKO_AZ.....UBE2HKO., 0.20, na.rm = TRUE) #bottom 20 of protein <= this value
MS2_UBEAZ.UBE_Up <- quantile(MS2$Abundance.Ratio..log2....UBE2HKO_AZ.....UBE2HKO., 0.80, na.rm = TRUE) #Top 20 of protein >= this value
...
MS.AZ.D$term_name
MS.AZ.D<-MS.AZ.D[c(1,2,19, 3,6,7, 5,9),]
...
### Plot Other genes ##### 
## check to see if UBE2H-KO genes have more of the downstream proteins usually degraded by UBE2H
# review: UBE2H and several subunits of CTLH are conserved between yeast, arabidopsis, and humans. In yeast UBC8 (UBE2H homolog) regulates glucose metabolism, but human CTLH regulates glycolysis. In humans in vitro experiments have identified substrates include glycolysis enzymes PKM2 and LDHA, energy regulator AMPK, and HBP1 a transcription factor that regulates cell cycle regulatory proteins.
# PRKAA2 (subunits of AMPK)

# no PKM2 gene in dataset
# energy regulator AMPK, (Not in dataset)
# HBP1 a transcription factor that regulates cell cycle regulatory proteins. HBP1 transcription factor all near 0. 

# CTLH complex: ARMC8, RUMY1, GID4, GID8, MAEA, MKLN1, RANBP9, RANBP10, RMND5A, RMND5B, WDR26, YPEL5
...
ggplot(MS2, aes(x=Abundance.Ratio..log2....UBE2HKO.....WT., 
  ...
  geom_point()+ 
  theme_classic()+
  geom_point(data = subset(MS2, Gene.Symbol...1 %in% c("HMGCS1", "HMGCS2", "PALLD", "VASP", "ACTL8", "MKLN1")), color = "red") + 
  #geom_point(data = subset(MS2, Gene.Symbol...1 %in% c("ARMC8", "RUMY1", "GID4", "GID8", "MAEA", "MKLN1", "RANBP9", "RANBP10", "RMND5A", "RMND5B", "WDR26", "YPEL5")), color = "blue") + #CTLH
  geom_density2d(color="white")+
  xlab(paste("Abundance Ratio (log2): UBE2H-KO/WT"))+
  ylab(paste("-log2(p-value)"))
# plot.MS.UBE2HKO.WT.TopHits.pdf
# Figure 6A
...
ggplot(MS2, aes(x=Abundance.Ratio..log2....UBE2HKO_AZ.....AZ., 
  ...
  theme_classic()+ FMN1
  geom_point(data = subset(MS2, Gene.Symbol...1 %in% c("HMGCS1", "HMGCS2", "PALLD", "VASP", "ACTL8", "MKLN1")), color = "red") + 
  #geom_point(data = subset(MS2, Gene.Symbol...1 %in% c("VASP")), color = "red") + 
  geom_density2d(color="white")+
  xlab(paste("Abundance Ratio (log2): UBE2H-KO+AZ / WT+AZ"))+
  ylab(paste("-log2(p-value)"))
# plot.MS.UBE2HKOAZ.WTAZ.TopHits.pdf
  #Figure S6G
## Genes that are upregulated in UBE2H-KO and UBE2H-KO AZ: 
# subset(MS2, Abundance.Ratio..log2....UBE2HKO.....WT. > 2 & Abundance.Ratio..log2....UBE2HKO_AZ.....AZ. > 2 & Abundance..F12..Sample..UBE2HKO > 1000)$Gene.Symbol...1
# "HBA"    "HBD"     "HBB"    "SOD1"   "VASP"   "FABP6"   "HMGCS1" "HMGCS2"   "MCM10"  "MKLN1"  "PALLD"

subset(MS2, Abundance.Ratio..log2....UBE2HKO.....WT. > 2.5 & Abundance.Ratio..log2....UBE2HKO_AZ.....AZ. > 2.5 & Abundances..Grouped...UBE2HKO > 1000)$Gene.Symbol...1


# Hemogloblin subunits?? "HBA"  ,  "HBD"  ,   "HBB" , "HBE1", "HBG2" ,"HBG1","HBE1" 
# lipid metabolism ketogenesis:  "HMGCS1" "HMGCS2" 
# reduces free radicals: SOD1
# Actin regulation, cytoskeletoin, focal adhesion: VASP
# CTLH complex, cell shape, cell-matrix adhesion, actin cytoskeleton organization, regulation of receptor internalization, and signal transduction: MKLN1
# Actin cytoskeleton: PALLD      cytoskeletal scaffolding protein required for organizing the actin cytoskeleton. It localizes to the cytoskeleton, focal adhesions, lamellipodia, and Z disc, placing it at sites where cell architecture and adhesion are actively remodeled


# Actin: PALLD, MKLN1, VASP, ACTL8
# Cytoskeleton: SPDL1
# Metabolism: NMNAT1, HMGCS1, HMGCS2, KIAA1191


# NMNAT1 (component for NAD+ metabolism pathway)
# nuclear NAD(+) synthase
NMNAT1MS<- subset(MS2, Gene.Symbol...1=="NMNAT1")
NMNAT1MS<- t(NMNAT1MS)
NMNAT1MS[is.na(NMNAT1MS)] <- 0
NMNAT1MS<- NMNAT1MS[18:29]

PlotNMNAT1MS<- data.frame(Sample =  factor(c("AZ3146", "AZ3146", "AZ3146", "UBE2H-KO", "UBE2H-KO", "UBE2H-KO", 
                                          "UBE2HKO + AZ3145", "UBE2HKO + AZ3145", "UBE2HKO + AZ3145", "WT", "WT", "WT"), level=c("WT", "AZ3146", "UBE2H-KO", "UBE2HKO + AZ3145")),
                       Abundance =  as.numeric(NMNAT1MS)) 
PlotNMNAT1MS

ggplot(PlotNMNAT1MS, aes(x=Sample, 
                      y= Abundance, fill= Sample ))+
  geom_boxplot()+
  scale_fill_manual(values= c("turquoise2","turquoise2","mediumorchid2","mediumorchid2"))+
  geom_point()+
  ...
  theme(axis.text.x = element_text(angle = 45, hjust=1))+ 
  xlab(paste(""))+
  ylab(paste("NMNAT1 Abundance"))+
  ggtitle(paste("NMNAT1 Abundance per sample"))
# plot.MS.NMNAT1_Abundance.pdf
## KO Has much higher NMNAT1!!! UBE2H degrades it! 
subset(MS2, Gene.Symbol...1=="NMNAT1")$Abundance.Ratio.Adj..P.Value...UBE2HKO.....WT.     #0.4758754
subset(MS2, Gene.Symbol...1=="NMNAT1")$Abundance.Ratio.Adj..P.Value...UBE2HKO_AZ.....AZ.  #0.2442424



# HMGCS1 (component for nutrient sensing and cholesterol metabolism ).    !!!!!
# # involved in lipid metabolism, mevalonate pathway, and acetyl-CoA metabolism
HMGCS1MS<- subset(MS2, Gene.Symbol...1=="HMGCS1")
HMGCS1MS<- t(HMGCS1MS)
HMGCS1MS[is.na(HMGCS1MS)] <- 0
HMGCS1MS<- HMGCS1MS[18:29]

PlotHMGCS1MS<- data.frame(Sample =  factor(c("AZ3146", "AZ3146", "AZ3146", "UBE2H-KO", "UBE2H-KO", "UBE2H-KO", 
                                             "UBE2HKO + AZ3145", "UBE2HKO + AZ3145", "UBE2HKO + AZ3145", "WT", "WT", "WT"), level=c("WT", "AZ3146", "UBE2H-KO", "UBE2HKO + AZ3145")),
                          Abundance =  as.numeric(HMGCS1MS)) 
PlotHMGCS1MS

ggplot(PlotHMGCS1MS, aes(x=Sample, 
                         y= Abundance, fill= Sample ))+
  geom_boxplot()+
  scale_fill_manual(values= c("turquoise2","turquoise2","mediumorchid2","mediumorchid2"))+
  geom_point()+
  ...
  theme(axis.text.x = element_text(angle = 45, hjust=1))+ 
  xlab(paste(""))+
  ylab(paste("HMGCS1 Abundance"))+
  ggtitle(paste("HMGCS1 Abundance per sample"))
# plot.MS.HMGCS1_Abundance.pdf
# Figure S6F
## KO Has much higher HMGCS1!!! 
subset(MS2, Gene.Symbol...1=="HMGCS1")$Abundance.Ratio.Adj..P.Value...UBE2HKO.....WT.     #8.387464e-15
subset(MS2, Gene.Symbol...1=="HMGCS1")$Abundance.Ratio.Adj..P.Value...UBE2HKO_AZ.....AZ.  #1.967647e-16


# HMGCS2 (component for nutrient sensing and cholesterol metabolism ).  In Mitochondria!  !!!!!!!
# mitochondrial enzyme that catalyzes the first reaction of ketogenesis, linking lipid-derived carbon to energy production during carbohydrate deprivation such as fasting
# hepatobiliary system and gastrointestinal tract
# Located in mitochondria and mitochondrial matrix
HMGCS2MS<- subset(MS2, Gene.Symbol...1=="HMGCS2")
HMGCS2MS<- t(HMGCS2MS)
HMGCS2MS[is.na(HMGCS2MS)] <- 0
HMGCS2MS<- HMGCS2MS[18:29]

PlotHMGCS2MS<- data.frame(Sample =  factor(c("AZ3146", "AZ3146", "AZ3146", "UBE2H-KO", "UBE2H-KO", "UBE2H-KO", 
                                             "UBE2HKO + AZ3145", "UBE2HKO + AZ3145", "UBE2HKO + AZ3145", "WT", "WT", "WT"), level=c("WT", "AZ3146", "UBE2H-KO", "UBE2HKO + AZ3145")),
                          Abundance =  as.numeric(HMGCS2MS)) 
PlotHMGCS2MS

ggplot(PlotHMGCS2MS, aes(x=Sample, y= Abundance, fill=Sample) )+
  geom_boxplot()+
  scale_fill_manual(values= c("turquoise2","turquoise2","mediumorchid2","mediumorchid2"))+
  geom_point()+
  theme_classic()+
  theme(axis.text.x = element_text(angle = 45, hjust=1))+ 
  xlab(paste(""))+
  ylab(paste("HMGCS2 Abundance"))+
  ggtitle(paste("HMGCS2 Abundance per sample"))
# plot.MS.HMGCS2_Abundance.pdf
# Figure S6F
## KO Has much higher HMGCS2!!! 
# Mitochondrial protein
subset(MS2, Gene.Symbol...1=="HMGCS2")$Abundance.Ratio.Adj..P.Value...UBE2HKO.....WT.     #1.831659e-06
subset(MS2, Gene.Symbol...1=="HMGCS2")$Abundance.Ratio.Adj..P.Value...UBE2HKO_AZ.....AZ.   #4.663452e-11




## VASP is also a top UBE2H-KO overexpression gene
## Gene is involved in actin cytoskeleton
# controls F-actin
# localized to contractile ring in cytokinsis

VASPMS<- subset(MS2, Gene.Symbol...1=="VASP")
VASPMS<- t(VASPMS)
VASPMS[is.na(VASPMS)] <- 0
VASPMS<- VASPMS[18:29]

PlotVASPMS<- data.frame(Sample =  factor(c("AZ3146", "AZ3146", "AZ3146", "UBE2H-KO", "UBE2H-KO", "UBE2H-KO", 
                                             "UBE2HKO + AZ3145", "UBE2HKO + AZ3145", "UBE2HKO + AZ3145", "WT", "WT", "WT"), level=c("WT", "AZ3146", "UBE2H-KO", "UBE2HKO + AZ3145")),
                          Abundance =  as.numeric(VASPMS)) 
PlotVASPMS

ggplot(PlotVASPMS, aes(x=Sample, 
                         y= Abundance, fill= Sample ))+
  geom_boxplot()+
  scale_fill_manual(values= c("turquoise2","turquoise2","mediumorchid2","mediumorchid2"))+
  geom_point()+
  ...
  theme(axis.text.x = element_text(angle = 45, hjust=1))+ 
  xlab(paste(""))+
  ylab(paste("VASP Abundance"))+
  ggtitle(paste("VASP Abundance per sample"))
# plot.MS.VASP_Abundance.pdf
# Figure S6F
## KO Has much higher VASP!!! UBE2H degrades it?
subset(MS2, Gene.Symbol...1=="VASP")$Abundance.Ratio.Adj..P.Value...UBE2HKO.....WT.     #1.893557e-16
subset(MS2, Gene.Symbol...1=="VASP")$Abundance.Ratio.Adj..P.Value...UBE2HKO_AZ.....AZ.   #8.714807e-15




## PALLD actin
PALLDMS<- subset(MS2, Gene.Symbol...1=="PALLD")
PALLDMS<- t(PALLDMS)
PALLDMS[is.na(PALLDMS)] <- 0
PALLDMS<- PALLDMS[18:29]
PlotPALLDMS<- data.frame(Sample =  factor(c("AZ3146", "AZ3146", "AZ3146", "UBE2H-KO", "UBE2H-KO", "UBE2H-KO", 
                                           "UBE2HKO + AZ3145", "UBE2HKO + AZ3145", "UBE2HKO + AZ3145", "WT", "WT", "WT"), level=c("WT", "AZ3146", "UBE2H-KO", "UBE2HKO + AZ3145")),
                        Abundance =  as.numeric(PALLDMS)) 
ggplot(PlotPALLDMS, aes(x=Sample, 
                       y= Abundance, fill= Sample ))+
  geom_boxplot()+
  scale_fill_manual(values= c("turquoise2","turquoise2","mediumorchid2","mediumorchid2"))+
  geom_point()+
  ...
  theme(axis.text.x = element_text(angle = 45, hjust=1))+ 
  xlab(paste(""))+
  ylab(paste("PALLD Abundance"))+
  ggtitle(paste("PALLD Abundance per sample"))
# plot.MS.PALLD_Abundance.pdf
# Figure S6F
## KO Has much higher PALLD!!! UBE2H degrades it?
subset(MS2, Gene.Symbol...1=="PALLD")$Abundance.Ratio.Adj..P.Value...UBE2HKO.....WT.     #5.550304e-07
subset(MS2, Gene.Symbol...1=="PALLD")$Abundance.Ratio.Adj..P.Value...UBE2HKO_AZ.....AZ.   #4.112946e-07




## MKLN1 actin
MKLN1MS<- subset(MS2, Gene.Symbol...1=="MKLN1")
MKLN1MS<- t(MKLN1MS)
MKLN1MS[is.na(MKLN1MS)] <- 0
MKLN1MS<- MKLN1MS[18:29]
PlotMKLN1MS<- data.frame(Sample =  factor(c("AZ3146", "AZ3146", "AZ3146", "UBE2H-KO", "UBE2H-KO", "UBE2H-KO", 
                                            "UBE2HKO + AZ3145", "UBE2HKO + AZ3145", "UBE2HKO + AZ3145", "WT", "WT", "WT"), level=c("WT", "AZ3146", "UBE2H-KO", "UBE2HKO + AZ3145")),
                         Abundance =  as.numeric(MKLN1MS)) 
ggplot(PlotMKLN1MS, aes(x=Sample, 
                        y= Abundance, fill= Sample ))+
  geom_boxplot()+
  scale_fill_manual(values= c("turquoise2","turquoise2","mediumorchid2","mediumorchid2"))+
  geom_point()+
  ...
  theme(axis.text.x = element_text(angle = 45, hjust=1))+ 
  xlab(paste(""))+
  ylab(paste("MKLN1 Abundance"))+
  ggtitle(paste("MKLN1 Abundance per sample"))
# plot.MS.MKLN1_Abundance.pdf
# Figure S6F
## KO Has much higher MKLN1!!! UBE2H degrades it! 
subset(MS2, Gene.Symbol...1=="MKLN1")$Abundance.Ratio.Adj..P.Value...UBE2HKO.....WT.     #1.222313e-07
subset(MS2, Gene.Symbol...1=="MKLN1")$Abundance.Ratio.Adj..P.Value...UBE2HKO_AZ.....AZ.    #2.172372e-06



## ACTL8 actin
ACTL8MS<- subset(MS2, Gene.Symbol...1=="ACTL8")
ACTL8MS<- t(ACTL8MS)
ACTL8MS[is.na(ACTL8MS)] <- 0
ACTL8MS<- ACTL8MS[18:29]
PlotACTL8MS<- data.frame(Sample =  factor(c("AZ3146", "AZ3146", "AZ3146", "UBE2H-KO", "UBE2H-KO", "UBE2H-KO", 
                                            "UBE2HKO + AZ3145", "UBE2HKO + AZ3145", "UBE2HKO + AZ3145", "WT", "WT", "WT"), level=c("WT", "AZ3146", "UBE2H-KO", "UBE2HKO + AZ3145")),
                         Abundance =  as.numeric(ACTL8MS)) 
ggplot(PlotACTL8MS, aes(x=Sample, 
                        y= Abundance , fill= Sample))+
  geom_boxplot()+
  scale_fill_manual(values= c("turquoise2","turquoise2","mediumorchid2","mediumorchid2"))+
  geom_point()+
  ...
  theme(axis.text.x = element_text(angle = 45, hjust=1))+ 
  xlab(paste(""))+
  ylab(paste("ACTL8 Abundance"))+
  ggtitle(paste("ACTL8 Abundance per sample"))
# plot.MS.ACTL8_Abundance.pdf
# Figure S6F
## KO Has much higher ACTL8!!! UBE2H degrades it?
subset(MS2, Gene.Symbol...1=="ACTL8")$Abundance.Ratio.Adj..P.Value...UBE2HKO.....WT.     #1.893557e-16
subset(MS2, Gene.Symbol...1=="ACTL8")$Abundance.Ratio.Adj..P.Value...UBE2HKO_AZ.....AZ.    #1.967647e-16
