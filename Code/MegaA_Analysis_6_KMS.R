...
ggplot(MegaA.All.Merge.QN_meanDiff, aes(y=-log(p.paired,2), x=Paired.Diff, 
                                        color=p.paired<0.05 & Paired.Diff< -0.2 & MeanBeta <0))+
  geom_point()+
...
# plot.BetaDelta.pValue.AllMegaA_QN_July2025.pdf
# PSMD14, USP49, PA2G4
# ehhh.. very few pathway genes. hard to make concludions
...
## Merge extra genes with alternative names between WGL and DG
...
# Since A2780 has several unreliable outliers (NA), turn outliers into "0" 
...
setwd(ResultsFile)
setwd("/Volumes/Schukken_SSD/CRISPR Screen/Overview_All")
...
ggpaired(df, cond1 = "Control", cond2 = "Aneuploid", 
  ...
  scale_fill_manual(values=c("skyblue", "yellow2"))
# 3x5
# plot.paired.MegaA.WGL_FOSL1.pdf
# Supplementary Figure S4B
t.test(as.numeric(subset(Merge.ALL.QN2, Gene==GeneOfInterest)[,c(1, 3,3, 6,6,6, 10,10, 13,13, 16,16, 19,19, 22,22,22,22,   27, 29, 32, 34,35, 37, 39)]),
       as.numeric(subset(Merge.ALL.QN2, Gene==GeneOfInterest)[,c(2, 4,5, 7,8,9, 11,12, 14,15, 17,18, 20,21, 23,24,25,26, 28, 30, 31, 33,33, 36, 38)])  ,
       paired=TRUE) # p= 0.003542, difference = 0.2278375
...
HighlightTheseGenes<- subset(Merge.Beta.WGL.Mega1, p.value< 0.05 & MeanDiff< -0.2 & MeanBeta <0 )$Gene
...
HighlightTheseGenes<- c("FOSL1", "VPS18", "ANAPC11")
...
ggplot(Merge.Beta.WGL.Mega1, aes(MeanDiff, -log2(p.value)) ) +
  ...
  xlab("MegaA + WGL data (Sheltzer) \nBeta Delta (Aneuploid - Euploid)")+
  ylab("-log2 (p-value)")
# plot.MegaA.WGL.BetaDelta.pvalue_UBE2H.pdf
...
Merge.ALL.QN3$MeanBeta <- rowMeans(Merge.ALL.QN3[,2:40])
...
ggplot(Merge.ALL.QN3, aes(x= ANvEU.Beta.Paired, 
                                 y= -log2(ANvEU.P.Paired)))+
  geom_point()+
  geom_density_2d(color = "white", bins = 10)+
  ylab("-log2(P-value)")+
  xlab("Difference in Beta score\nAneuploid - euploid")+
  geom_point(data= subset(Merge.ALL.QN3, ANvEU.P.Paired<0.05 & ANvEU.Beta.Paired< -0.2 & MeanBeta <0 ), color = "red")+
  #scale_color_manual(values = c("Black", "red"), name = "P<0.05 \nDiff < -.2")+
  # geom_point(data = subset(Merge.ALL.QN3, Gene%in% c("UBE2H")), color="blue")+
  theme_classic()
# 5x4
# plot.BetaDelta.ANvEU_WGL.MegaA_June2025_MeanBeta.pdf
...
ggplot(Merge.ALL.QN3, aes(x= ANvEU.Beta.Paired, 
  ...
  theme_classic()
# 5x4
# plot.BetaDelta.MeanBeta.ANvEU_WGL.MegaA.pdf
...
HighlightTheseGenes<- c("UBE2H", "FOSL1") 
...
# Merge.ALL.QN3<- Merge.ALL.QN3[-1,]
    ...
    #### Cluster: Does paired difference correct for library, cell line and batch effect? #####
Merge.ALL.QN4<-Merge.ALL.QN3
Merge.ALL.QN4$HCT116.Ts8.Diff<- Merge.ALL.QN4$HCT116.Ts8.beta - Merge.ALL.QN4$HCT116.Cont1.beta
Merge.ALL.QN4$HCT116.Ts5.Diff<- Merge.ALL.QN4$HCT116.Tet5p.Ts5q.beta - Merge.ALL.QN4$HCT116.Cont2.beta
Merge.ALL.QN4$HCT116.Ts5..Diff<- Merge.ALL.QN4$HCT116.Tet5.beta - Merge.ALL.QN4$HCT116.Cont2.beta

Merge.ALL.QN4$DLD1.Ts13.Diff<- Merge.ALL.QN4$DLD1.Ts13.beta - Merge.ALL.QN4$DLD1.Cont1.beta
Merge.ALL.QN4$DLD1.Ts8.10.Diff<- Merge.ALL.QN4$DLD1.Ts8.10.beta - Merge.ALL.QN4$DLD1.Cont1.beta
Merge.ALL.QN4$DLD1.Ts2.18.Diff<- Merge.ALL.QN4$DLD1.Ts2.18.beta - Merge.ALL.QN4$DLD1.Cont1.beta
Merge.ALL.QN4$DLD1.Ts10.21.Diff<- Merge.ALL.QN4$DLD1.Ts10.21.beta - Merge.ALL.QN4$DLD1.Cont3.beta
Merge.ALL.QN4$DLD1.Ts5.15.Diff<- Merge.ALL.QN4$DLD1.Ts5.15.beta - Merge.ALL.QN4$DLD1.Cont3.beta
Merge.ALL.QN4$DLD1.Ts2.18..Diff<- Merge.ALL.QN4$DLD1.Ts2.18.beta_Try2 - Merge.ALL.QN4$DLD1.Cont4.beta
Merge.ALL.QN4$DLD1.Ts12.17.Diff<- Merge.ALL.QN4$DLD1.Ts12.17.beta - Merge.ALL.QN4$DLD1.Cont4.beta

Merge.ALL.QN4$A2780.Ts2.Diff<- Merge.ALL.QN4$A2780.Ts2.beta - Merge.ALL.QN4$A2780.Cont1.beta
Merge.ALL.QN4$A2780.Ts10.Diff<- Merge.ALL.QN4$A2780.Ts10.beta - Merge.ALL.QN4$A2780.Cont1.beta
Merge.ALL.QN4$A2780.Ts8.Diff<- Merge.ALL.QN4$A2780.Ts8.beta - Merge.ALL.QN4$A2780.Cont2.beta
Merge.ALL.QN4$A2780.Ts18.Diff<- Merge.ALL.QN4$A2780.Ts18.beta - Merge.ALL.QN4$A2780.Cont2.beta

Merge.ALL.QN4$Snu1.Ts10.17.5.Ps2.Diff<- Merge.ALL.QN4$Snu1.Ts10.17.5_Ps2.beta - Merge.ALL.QN4$Snu1.Cont.beta
Merge.ALL.QN4$Snu1.Ts10.1.8.14.Ps5.Diff<- Merge.ALL.QN4$Snu1.Ts10.1.8.14_Ps5.beta - Merge.ALL.QN4$Snu1.Cont.beta
Merge.ALL.QN4$Snu1.Ts10.6q.Ps8.12.Diff<- Merge.ALL.QN4$Snu1.Ts10.6q_Ps8.12.beta - Merge.ALL.QN4$Snu1.Cont.beta
Merge.ALL.QN4$Snu1.Ts9.19.Diff<- Merge.ALL.QN4$Snu1.Ts9.19.beta - Merge.ALL.QN4$Snu1.Cont.beta

Merge.ALL.QN4$WGL.DLD1.Ts13.Diff<- Merge.ALL.QN4$WGL.DLD1.Ts13.beta - Merge.ALL.QN4$WGL.DLD1.Cont.beta
Merge.ALL.QN4$WGL.VACO432.Ts13.Diff<- Merge.ALL.QN4$WGL.Vaco432.Ts13.beta - Merge.ALL.QN4$WGL.Vaco432.Cont.beta
Merge.ALL.QN4$WGL.A2780.Ts1q.Diff<- Merge.ALL.QN4$WGL.A2780.Cont.beta - Merge.ALL.QN4$WGL.A2780.Di1q.beta
Merge.ALL.QN4$WGL.A2058.Ts1q.Diff<- Merge.ALL.QN4$WGL.A2058.Cont.beta - Merge.ALL.QN4$WGL.A2058.Di1q.beta
Merge.ALL.QN4$WGL.A2058.Ts7p.Diff<- Merge.ALL.QN4$WGL.A2058.Cont.beta - Merge.ALL.QN4$WGL.A2058.Di7p.beta
Merge.ALL.QN4$WGL.AGS.Ts1q.Diff<- Merge.ALL.QN4$WGL.AGS.Cont.beta - Merge.ALL.QN4$WGL.AGS.Di1q.beta
Merge.ALL.QN4$WGL.MCF10A.Ts1q.Diff<- Merge.ALL.QN4$WGL.MCF10A.Cont.beta - Merge.ALL.QN4$WGL.MCF10A.1qDi.beta

### Cluster plot for difference in gene dropout: 
# Dendogram sample clustering 
col<- colorRampPalette(c("blue", "white", "red"))(20)       
heatmap(x = as.matrix(Merge.ALL.QN4[,c(48:72)]), col = col)  
# No longer see batch specific clustering
# plot.Branch.heatmap_Differnce.pdf
# 12x12
# Supplementary Figure 1E

# paired differences corrected for batch effects
...
#Merge.ALL.QN.Corr<- read.csv("Merge.ALL.QN.Corr.csv")
...
DepMap.Sheltzer.CRISPR_AneuScore<- subset(DepMap.Sheltzer.CRISPR_AneuScore, SampleNum > 200)
...
ggplot(DepMap.Sheltzer.CRISPR_AneuScore, aes(x=AneuScore.Corr, y=Aneu.divPloidy.CRISPR.Corr.Coef ))+
  ...
  geom_point(data = subset(DepMap.Sheltzer.CRISPR_AneuScore, 
                           AneuScore.Corr < -0.1  & Aneu.divPloidy.CRISPR.Corr.Coef < -0.1 & 
                             Aneu.divPloidy.CRISPR.Corr.P< 0.005 & AneuScore.Corr.P <0.1 & 
                             MeanBeta < 0), color = "red4") + 
  geom_point(data = subset(DepMap.Sheltzer.CRISPR_AneuScore, 
                           AneuScore.Corr < -0.1  & Aneu.divPloidy.CRISPR.Corr.Coef < -0.1 & 
                             Aneu.divPloidy.CRISPR.Corr.P< 0.005 & AneuScore.Corr.P <0.05 & 
                             MeanBeta <0 ), color = "red") + 
  ...
  scale_color_manual(values= c("Black", "Red"), name="Top hits")
# 5x4
# plot.DepMap.Sheltzer.CRISPR_MegaAWGLAneuScore_DepMapAneudivPloidy_MeanBeta0.pdf
...
ggplot(Merge.ALL.QN.lm, aes(AneuScore.lm, -log2(AneuScore.lm.P))) + 
  ...
  geom_vline(xintercept = 0)+
  geom_point()+
  geom_point(data = subset(Merge.ALL.QN.lm, Gene %in% c("FOSL1", "UBE2H")), color = "Blue") + 
...
# top hit: PKMYT1
# But UBE2H and FOSL1 are also top hits
...
ggplot(DepMap.CRISPR_AneuScore_MegaWGLTtest, aes(x=ANvEU.Beta.Paired, y=Aneu.divPloidy.CRISPR.Corr.Coef))+
  ...
  #                           Aneu.divPloidy.CRISPR.Corr.P< 0.005 & ANvEU.P.Paired< 0.1 & 
  #                           MeanBeta <0), color = "red4")+
  geom_point(data = subset(DepMap.CRISPR_AneuScore_MegaWGLTtest, 
                           ANvEU.Beta.Paired < -0.2 & Aneu.divPloidy.CRISPR.Corr.Coef < -0.1 & 
                             Aneu.divPloidy.CRISPR.Corr.P< 0.005 & ANvEU.P.Paired< 0.05 &
                             MeanBeta<0), color = "red")+
...
#Highlight Genes located near UBE2H on chromosome 7q: 
# Genes near UBE2H do not show an aneuploid dependency trend. 
ggplot(DepMap.CRISPR_AneuScore_MegaWGLTtest, aes(x=ANvEU.Beta.Paired, y=Aneu.divPloidy.CRISPR.Corr.Coef))+
  geom_point()+
  #geom_point(data = subset(DepMap.CRISPR_AneuScore_MegaWGLTtest, ANvEU.Beta.Paired < -0.15 & Aneu.divPloidy.CRISPR.Corr.Coef < -0.15), color = "red")+
  geom_point(data = subset(DepMap.CRISPR_AneuScore_MegaWGLTtest, Gene %in% 
                             c("NRF1", "STRIP2", "SMKR1",  "AHCYL2","SMO", "TSPAN33",
                               "TPNO3", "IRF5", "KCP","FLNC","CCDC136", "CALU", "OPN1SW", "GARIN1B", "GARIN1A",
                               "ZC3HC1", "TMEM209", "KLHDC10",
                               "CPA1","CPA2","CPA4","CPA5", "CEP41","MEST", "SSMEM1",
                               "COPG2", "KLF14", "TSGA13","TNPO3")), color = "red")+
  theme_classic()+
  geom_hline(yintercept = 0)+
  geom_vline(xintercept = 0)+
  geom_density_2d(color = "white", bins = 10)+
  ...
  scale_color_manual(values= c("Black", "Red"), name="Top hits")
# 5x4
# plot.DepMap.CRISPR_AneuScore_MegaWGL_7qGenesNearUBE2H.pdf
...
GeneList10p<- unique(subset(Protein_Info3, ChrmArm=="10p")$`Approved Symbol`)
GeneList10q<- unique(subset(Protein_Info3, ChrmArm=="10q")$`Approved Symbol`)
GeneList10<- c(GeneList10q, GeneList10p)
...
Merge.Ts10.QN3$meanBeta<- rowMeans(Merge.Ts10.QN3[,1:6])



# Now correct for chrm 10 extra toxicity due to Cas9 increasing cuts 
## Now calculate Beta score correcting for chromosome trisomy dependency: 
# mean difference in Beta score for genes on chrm 10: 
GeneList10p<- unique(subset(Protein_Info3, ChrmArm=="10p")$`Approved Symbol`)
GeneList10q<- unique(subset(Protein_Info3, ChrmArm=="10q")$`Approved Symbol`)
GeneList10<- c(GeneList10q, GeneList10p)

mean(subset(Merge.Ts10.QN3, !Gene%in% GeneList10)$ANvEU.Beta) # -0.0001109906
mean(subset(Merge.Ts10.QN3, Gene%in% GeneList10)$ANvEU.Beta)  # 0.02171538
MeanTs10Diff<- mean(subset(Merge.Ts10.QN3, Gene%in% GeneList10)$ANvEU.Beta) - 
  mean(subset(Merge.Ts10.QN3, ! Gene%in% GeneList10)$ANvEU.Beta)

Merge.Ts10.QN3$Ts10CorrectedDiff<- Merge.Ts10.QN3$ANvEU.Beta
for(i in 1: length(Merge.Ts10.QN3$Gene)){
  if (Merge.Ts10.QN3$Gene[i] %in% GeneList10){
    Merge.Ts10.QN3$Ts10CorrectedDiff[i]<- Merge.Ts10.QN3$ANvEU.Beta[i] - MeanTs10Diff
  }
}


# now plot
Mega.WGL.Ts10.Only.TopHits<- subset(Merge.Ts10.QN3, ANvEU.P<0.05 & Ts10CorrectedDiff< -0.2 &  meanBeta<0)$Gene

ggplot(Merge.Ts10.QN3, aes(x= Ts10CorrectedDiff, y= -log2(ANvEU.P), 
                           color= Gene %in% Mega.WGL.Ts10.Only.TopHits))+
  geom_point()+
  ylab("-log2(P-value)")+
  geom_density_2d(color = "white", bins = 10)+
  xlab("Difference in Beta score\n Gain 10 - Neutral 10")+
...
subset(Merge.Ts10.QN3, ANvEU.P<0.05 & Ts10CorrectedDiff< -0.2 & meanBeta<0)$Gene


### Look at genes located ON chromosome 10: 
Chrm10Genes<- unique(subset(Gene_Info, `Chromosome/scaffold name` == 10)$`HGNC symbol`)
TopHits<- Chrm10Genes
ggplot(Merge.Ts10.QN3, aes(x= Ts10CorrectedDiff, 
...
ggplot(Merge.Ts10.QN3, aes(x= Ts10CorrectedDiff, 
  ...
  theme_classic()
# 5x4
# plot.BetaDelta.Ts10vEU.colorDepMap_SigDif_10corrected.pdf
...
# G-Profiler: 
g.WGL.Ts10.Toxic<- gost(
  subset(Merge.Ts10.QN3, ANvEU.Beta < -0.2)$Gene,
  organism = "hsapiens",
  ordered_query = FALSE,
  multi_query = FALSE,
  significant = TRUE,
  ...
  domain_scope = "custom",
  custom_bg = as.character(Merge.Ts10.QN3$Gene),
  numeric_ns = "",
  sources = NULL,
  as_short_link = FALSE
)

g.WGL.Ts10.Toxic_result<-g.WGL.Ts10.Toxic$result
g.WGL.Ts10.Toxic_result<-g.WGL.Ts10.Toxic_result[order(g.WGL.Ts10.Toxic_result$p_value),]
g.WGL.Ts10.Toxic_result

setwd(ResultsFile)
# write.csv(g.WGL.Ts10.Toxic_result[,1:13], "gProfiler.WGL.Ts10Toxic.Sheltzer.csv")


##Toxic to aneuploid cells Sheltzer
P.10Tox<-g.WGL.Ts10.Toxic_result[order(g.WGL.Ts10.Toxic_result$p_value),]
P.10Tox$term_name<- factor(P.10Tox$term_name, levels= P.10Tox$term_name)
P.10Tox$Termtype<-c("Cell Cycle", "Cell Cycle"
)
...
ggplot(P.10Tox, 
  ...
  ggtitle ("Biological terms enriched: Ts10 toxic (Beta-Score)")+
  theme_classic()+
  coord_flip()
## 
# plot.gProfiler.sheltzer.Mega.Ts10.pdf
# 10x5
...
Merge.Ts8.QN2$meanBeta<- rowMeans(Merge.Ts8.QN2[,1:8])


# Now correct for chrm 8 extra toxicity due to Cas9 increasing cuts 
## Now calculate Beta score correcting for chromosome trisomy dependency: 
# mean difference in Beta score for genes on chrm 8: 
GeneList8p<- unique(subset(Protein_Info3, ChrmArm=="8p")$`Approved Symbol`)
GeneList8q<- unique(subset(Protein_Info3, ChrmArm=="8q")$`Approved Symbol`)
GeneList8<- c(GeneList8q, GeneList8p)

mean(subset(Merge.Ts8.QN2, !Gene%in% GeneList8)$ANvEU.Beta) # -0.0009377402
mean(subset(Merge.Ts8.QN2, Gene%in% GeneList8)$ANvEU.Beta)  # 0.0002838049
MeanTs8Diff<- mean(subset(Merge.Ts8.QN2, Gene%in% GeneList8)$ANvEU.Beta) - 
  mean(subset(Merge.Ts8.QN2, ! Gene%in% GeneList8)$ANvEU.Beta)

Merge.Ts8.QN2$Ts8CorrectedDiff<- Merge.Ts8.QN2$ANvEU.Beta
for(i in 1: length(Merge.Ts8.QN2$Gene)){
  if (Merge.Ts8.QN2$Gene[i] %in% GeneList8){
    Merge.Ts8.QN2$Ts8CorrectedDiff[i]<- Merge.Ts8.QN2$ANvEU.Beta[i] - MeanTs8Diff
  }
}
# Ts8CorrectedDiff
...
subset(Merge.Ts8.QN2, ANvEU.P<0.05 & Ts8CorrectedDiff< -0.2)$Gene



x<- t.test(subset(Merge.Ts8.QN2, !Gene %in% GeneList8)$Ts8CorrectedDiff, 
           subset(Merge.Ts8.QN2, Gene %in% GeneList8)$Ts8CorrectedDiff)
# A2780 & AGS: P = NS 
...
ggplot(Merge.Ts8.QN2, aes(x= Ts8CorrectedDiff, 
                          y= -log2(ANvEU.P)))+
  geom_point()+
  ...
  theme_classic()
# 5x4
# plot.BetaDelta.Ts8vEU.colorDepMap_sigdif_8Corrected.pdf
# Figure 4B
...
Merge.Ts8.QN2$ANvEU.Beta < -0.2



# G-Profiler: 
g.WGL.Ts8.Toxic<- gost(
  subset(Merge.Ts8.QN2, ANvEU.Beta < -0.2)$Gene,
  organism = "hsapiens",
  ordered_query = FALSE,
  multi_query = FALSE,
  significant = TRUE,
  ...
  correction_method = c("g_SCS"),
  domain_scope = "custom",
  custom_bg = as.character(Merge.Ts8.QN2$Gene),
  numeric_ns = "",
  sources = NULL,
  as_short_link = FALSE
)

g.WGL.Ts8.Toxic_result<-g.WGL.Ts8.Toxic$result
g.WGL.Ts8.Toxic_result<-g.WGL.Ts8.Toxic_result[order(g.WGL.Ts8.Toxic_result$p_value),]
g.WGL.Ts8.Toxic_result 
...
##Toxic to aneuploid cells Sheltzer
P.8Tox<-g.WGL.Ts8.Toxic_result[order(g.WGL.Ts8.Toxic_result$p_value),]
P.8Tox$term_name<- factor(P.8Tox$term_name, levels= P.8Tox$term_name)
P.8Tox$Termtype<-c("Cell Cycle", "Cell Cycle"
)
...
ggplot(P.8Tox, 
  ...
  ylab("-log2 (P-Value)") +
  xlab("Key Biological terms")+
  ggtitle ("Biological terms enriched: Ts8 toxic (Beta-Score)")+
  theme_classic()+
  coord_flip()
## 
# plot.gProfiler.sheltzer.Mega.Ts8.pdf
# 10x5
...
# Now correct for chrm 5 extra toxicity due to Cas9 increasing cuts 
## Now calculate Beta score correcting for chromosome trisomy dependency: 
# mean difference in Beta score for genes on chrm 5: 
GeneList5p<- unique(subset(Protein_Info3, ChrmArm=="5p")$`Approved Symbol`)
GeneList5q<- unique(subset(Protein_Info3, ChrmArm=="5q")$`Approved Symbol`)
GeneList5<- c(GeneList5q, GeneList5p)

mean(subset(Merge.Ts5.QN2, !Gene%in% GeneList5)$ANvEU.Beta) # -
mean(subset(Merge.Ts5.QN2, Gene%in% GeneList5)$ANvEU.Beta)  # 
MeanTs5Diff<- mean(subset(Merge.Ts5.QN2, Gene%in% GeneList5)$ANvEU.Beta) - 
  mean(subset(Merge.Ts5.QN2, ! Gene%in% GeneList5)$ANvEU.Beta)

Merge.Ts5.QN2$Ts5CorrectedDiff<- Merge.Ts5.QN2$ANvEU.Beta
for(i in 1: length(Merge.Ts5.QN2$Gene)){
  if (Merge.Ts5.QN2$Gene[i] %in% GeneList5){
    Merge.Ts5.QN2$Ts5CorrectedDiff[i]<- Merge.Ts5.QN2$ANvEU.Beta[i] - MeanTs5Diff
  }
}
# Ts5CorrectedDiff


Mega.WGL.Ts5.Only.TopHits<- subset(Merge.Ts5.QN2, ANvEU.P<0.1 & Ts5CorrectedDiff< -0.2)$Gene
ggplot(Merge.Ts5.QN2, aes(x= Ts5CorrectedDiff, 
...
length(AntiJoinMegaATs5$Gene2) # 179
...
ggplot(DepMap.Sheltzer.CRISPR_Ts5p, aes(x=ANvEU.Beta, y=Gain5p.min.LossNeutral.Diff, 
                                        color= ANvEU.Beta < -0.2 & Gain5p.min.LossNeutral.Diff < -0.05))+
  geom_point()+
  geom_point(data = subset(DepMap.Sheltzer.CRISPR_Ts5p, ANvEU.Beta < -0.2 & Gain5p.min.LossNeutral.Diff < -0.05))+
  theme_classic()+
  geom_hline(yintercept = 0) +
  geom_vline(xintercept = 0) +
  geom_density_2d(color = "white", bins = 10)+
...
subset(DepMap.Sheltzer.CRISPR_Ts5p, ANvEU.Beta < -0.2 & Gain5p.min.LossNeutral.Diff < -0.05)#$Gene
...
ggplot(Merge.Ts5.QN2, aes(x= Ts5CorrectedDiff, 
  ...
  #geom_point(data = subset(Merge.Ts5.QN2, Gene %in% c("UBE2H", "FOSL1", "PKMYT1")), color = "blue")+ 
  ylab("-log2(P-value)")+
  geom_density_2d(color = "white", bins = 10)+
  xlab("Difference in Beta score\nTs5p - euploid")+
  theme_classic()
...
setwd(ResultsFile)
# write.csv(DepMap.Sheltzer.CRISPR_Ts5p, "Chrm5p_Specific_Toxicity_Sheltzer.DepMap.csv")
# DepMap.Sheltzer.CRISPR_Ts5p<- read.csv("Chrm5p_Specific_Toxicity_Sheltzer.DepMap.csv")


         ##### g:Profiler Ts5: p=0.03323576 DNA repair #####



# G-Profiler: 
g.WGL.Ts5.Toxic<- gost(
  subset(Merge.Ts5.QN2, ANvEU.Beta < -0.2)$Gene,
  organism = "hsapiens",
  ordered_query = FALSE,
  multi_query = FALSE,
  significant = TRUE,
  ...
  correction_method = c("g_SCS"),
  domain_scope = "custom",
  custom_bg = as.character(Merge.Ts5.QN2$Gene),
  numeric_ns = "",
  sources = NULL,
  as_short_link = FALSE
)

g.WGL.Ts5.Toxic_result<-g.WGL.Ts5.Toxic$result
g.WGL.Ts5.Toxic_result<-g.WGL.Ts5.Toxic_result[order(g.WGL.Ts5.Toxic_result$p_value),]
g.WGL.Ts5.Toxic_result

setwd(ResultsFile)
# write.csv(g.WGL.Ts5.Toxic_result[,1:13], "gProfiler.WGL.Ts5Toxic.Sheltzer.csv")


##Toxic to aneuploid cells Sheltzer
P.5Tox<-g.WGL.Ts5.Toxic_result[order(g.WGL.Ts5.Toxic_result$p_value),]
P.5Tox$term_name<- factor(P.5Tox$term_name, levels= P.5Tox$term_name)
P.5Tox$Termtype<-c("DNA repair")
...
ggplot(P.5Tox, 
  ...
  coord_flip()
## 
# plot.gProfiler.sheltzer.Mega.Ts5p.pdf
# 10x5
...
# Now correct for chrm 2 extra toxicity due to Cas9 increasing cuts 
## Now calculate Beta score correcting for chromosome trisomy dependency: 
# mean difference in Beta score for genes on chrm 2: 
GeneList2p<- unique(subset(Protein_Info3, ChrmArm=="2p")$`Approved Symbol`)
GeneList2q<- unique(subset(Protein_Info3, ChrmArm=="2q")$`Approved Symbol`)
GeneList2<- c(GeneList2q, GeneList2p)

mean(subset(Merge.Ts2.QN2, !Gene%in% GeneList2)$ANvEU.Beta) # -
mean(subset(Merge.Ts2.QN2, Gene%in% GeneList2)$ANvEU.Beta)  # 
MeanTs2Diff<- mean(subset(Merge.Ts2.QN2, Gene%in% GeneList2)$ANvEU.Beta) - 
  mean(subset(Merge.Ts2.QN2, ! Gene%in% GeneList2)$ANvEU.Beta)

Merge.Ts2.QN2$Ts2CorrectedDiff<- Merge.Ts2.QN2$ANvEU.Beta
for(i in 1: length(Merge.Ts2.QN2$Gene)){
  if (Merge.Ts2.QN2$Gene[i] %in% GeneList2){
    Merge.Ts2.QN2$Ts2CorrectedDiff[i]<- Merge.Ts2.QN2$ANvEU.Beta[i] - MeanTs2Diff
  }
}
# Ts2CorrectedDiff
...
Mega.WGL.Ts2.TopHits<- subset(Merge.Ts2.QN2, ANvEU.P<0.05 & Ts2CorrectedDiff< -0.2)$Gene
ggplot(Merge.Ts2.QN2, aes(x= Ts2CorrectedDiff, 
...
subset(Merge.Ts2.QN2, ANvEU.P<0.05 & Ts2CorrectedDiff< -0.2 )$Gene
...
ggplot(Merge.Ts2.QN2, aes(x= Ts2CorrectedDiff, 
  ...
  #geom_point(data = subset(Merge.Ts2.QN2, Gene %in% c("NRBP1","PSMB3")), color = "blue")+ 
  ylab("-log2(P-value)")+
  geom_density_2d(color = "white", bins = 10)+
  xlab("Difference in Beta score\nTs2 - euploid")+
  theme_classic()
# 5x4 2p
# plot.BetaDelta.Ts2vEU.colorDepMap_SigDif_Corrected2.pdf
# Figure 4B 
...
# G-Profiler: 
g.WGL.Ts2.Toxic<- gost(
  subset(Merge.Ts2.QN2, ANvEU.Beta < -0.2)$Gene,
  organism = "hsapiens",
  ordered_query = FALSE,
  multi_query = FALSE,
  significant = TRUE,
  ...
  correction_method = c("g_SCS"),
  domain_scope = "custom",
  custom_bg = as.character(Merge.Ts2.QN2$Gene),
  numeric_ns = "",
  sources = NULL,
  as_short_link = FALSE
)

g.WGL.Ts2.Toxic_result<-g.WGL.Ts2.Toxic$result
g.WGL.Ts2.Toxic_result<-g.WGL.Ts2.Toxic_result[order(g.WGL.Ts2.Toxic_result$p_value),]
g.WGL.Ts2.Toxic_result

setwd(ResultsFile)
# write.csv(g.WGL.Ts2.Toxic_result[,1:13], "gProfiler.WGL.Ts2Toxic.Sheltzer.csv")


##Toxic to aneuploid cells Sheltzer
P.2Tox<-g.WGL.Ts2.Toxic_result[order(g.WGL.Ts2.Toxic_result$p_value),]
P.2Tox$term_name<- factor(P.2Tox$term_name, levels= P.2Tox$term_name)
P.2Tox$Termtype<-c("DNA repair"
)
...
ggplot(P.2Tox, 
  ...
  ylab("-log2 (P-Value)") +
  xlab("Key Biological terms")+
  ggtitle ("Biological terms enriched: Ts2 toxic (Beta-Score)")+
  theme_classic()+
  coord_flip()
## 
# plot.gProfiler.sheltzer.Mega.Ts2.pdf
# 10x5
...
HighlightTheseGenes<- subset(MegaA.Merge.HCT116.QN_meanDiff, p.paired<0.05 & Paired.Diff< -0.2 & MeanBeta <0)$Gene
ggplot(MegaA.Merge.HCT116.QN_meanDiff, aes(y=-log(p.paired,2), x=Paired.Diff, color=Gene %in% HighlightTheseGenes))+
  ...
  scale_color_manual(values=c("Black", "Red"))+ 
  geom_hline(yintercept = 4.321928, color="grey50", linetype = "dashed")+ # log2(0.05) is 4.321928. dashed line cutoff at p< 0.05
  geom_vline(xintercept =-0.2, color="grey50", linetype = "dashed")+
  ...
  ylab("-log2(p-value)")
# 5x4
# plot.MegaA_HCT116.pairedP.Diff_Highlight_MeanBeta0.pdf
# Figure 4C
...
# write.csv(MegaA.Merge.HCT116.QN_meanDiff, "CRISPRScreens_BetaScores_MegaA_HCT116.csv")
          ...
          ##### g:Profiler HCT116: TF motif p= 0.01009072  #####



# G-Profiler: 
g.WGL.HCT116.Toxic<- gost(
  subset(MegaA.Merge.HCT116.QN_meanDiff, Paired.Diff < -0.2)$Gene,
  organism = "hsapiens",
  ordered_query = FALSE,
  multi_query = FALSE,
  significant = TRUE,
  ...
  correction_method = c("g_SCS"),
  domain_scope = "custom",
  custom_bg = as.character(MegaA.Merge.HCT116.QN_meanDiff$Gene),
  numeric_ns = "",
  sources = NULL,
  as_short_link = FALSE
)

g.WGL.HCT116.Toxic_result<-g.WGL.HCT116.Toxic$result
g.WGL.HCT116.Toxic_result
...
MegaA.Merge.DLD1.QN<- as.data.frame(MegaA.Merge.DLD1.QN)
...
## highlight hits
HighlightTheseGenes<- c("PRKDC")
HighlightTheseGenes<- subset(MegaA.Merge.DLD1.QN_meanDiff, p.paired<0.05 & Paired.Diff< -0.2 & MeanBeta <0)$Gene
ggplot(MegaA.Merge.DLD1.QN_meanDiff, aes(y=-log(p.paired,2), x=Paired.Diff, color=Gene %in% HighlightTheseGenes))+
  ...
  ylab("-log2(p-value)")
# 5x4
# plot.MegaA_DLD1.pairedP.Diff_Highlight_MeanBeta0.pdf
# Figure 4C

subset(MegaA.Merge.DLD1.QN_meanDiff, p.paired<0.05 & Paired.Diff< -0.2 & MeanBeta <0)
...
# G-Profiler: 
g.WGL.DLD1.Toxic<- gost(
  subset(MegaA.Merge.DLD1.QN_meanDiff, Paired.Diff < -0.2)$Gene,
  organism = "hsapiens",
  ordered_query = FALSE,
  multi_query = FALSE,
  significant = TRUE,
  ...
  correction_method = c("g_SCS"),
  domain_scope = "custom",
  custom_bg = as.character(MegaA.Merge.DLD1.QN_meanDiff$Gene),
  numeric_ns = "",
  sources = NULL,
  as_short_link = FALSE
)

g.WGL.DLD1.Toxic_result<-g.WGL.DLD1.Toxic$result
g.WGL.A2780.Toxic_result
...
MegaA.Merge.A2780.QN<- as.data.frame(MegaA.Merge.A2780.QN)
...
## Highlight hits
HighlightTheseGenes<- c("DSTYK")
HighlightTheseGenes <- subset(MegaA.Merge.A2780.QN_meanDiff, p.paired<0.05 & Paired.Diff< -0.2 & MeanBeta <0)$Gene
...
ggplot(MegaA.Merge.A2780.QN_meanDiff, aes(y=-log(p.paired,2), x=Paired.Diff, color=Gene %in% HighlightTheseGenes))+
  ...
  ylab("-log2(p-value)")
# 5x4
# plot.MegaA_A2780.pairedP.Diff_Highlight_MeanBeta0.pdf
# Figure 4C

subset(MegaA.Merge.A2780.QN_meanDiff, Gene %in% HighlightTheseGenes)


subset(MegaA.Merge.A2780.QN_meanDiff, p.paired<0.01 & Paired.Diff< -0.4 & MeanBeta <0)$Gene
...
# G-Profiler: 
g.WGL.A2780.Toxic<- gost(
  subset(MegaA.Merge.A2780.QN_meanDiff, Paired.Diff < -0.2)$Gene,
  organism = "hsapiens",
  ordered_query = FALSE,
  multi_query = FALSE,
  significant = TRUE,
  ...
  correction_method = c("g_SCS"),
  domain_scope = "custom",
  custom_bg = as.character(MegaA.Merge.A2780.QN_meanDiff$Gene),
  numeric_ns = "",
  sources = NULL,
  as_short_link = FALSE
)

g.WGL.A2780.Toxic_result<-g.WGL.A2780.Toxic$result
g.WGL.A2780.Toxic_result<-g.WGL.A2780.Toxic_result[order(g.WGL.A2780.Toxic_result$p_value),]
g.WGL.A2780.Toxic_result
...
MegaA.Merge.SNU1<- merge(Snu1.Cont1.C13.MegaA[,c(1,3)], Snu1.C12.MegaA[,c(1,3)], by=c("Gene", "Gene")) ### This one is wierd, low coverage leads to extreme negative beta scores
MegaA.Merge.SNU1<- merge(MegaA.Merge.SNU1, Snu1.C24.MegaA[,c(1,3)], by=c("Gene", "Gene"))
MegaA.Merge.SNU1<- merge(MegaA.Merge.SNU1, Snu1.C111.MegaA[,c(1,3)], by=c("Gene", "Gene"))
MegaA.Merge.SNU1<- merge(MegaA.Merge.SNU1, Snu1.C114.MegaA[,c(1,3)], by=c("Gene", "Gene"))
...
## Highlight hits
HighlightTheseGenes<-  c("UBC", "RPL23A")
HighlightTheseGenes<-  subset(MegaA.Merge.SNU1.QN_meanDiff, Paired.Diff< -0.2 & p.paired<0.05 & MeanBeta <0)$Gene
ggplot(MegaA.Merge.SNU1.QN_meanDiff, aes(y=-log(p.paired,2), x=Paired.Diff, color=Gene %in% HighlightTheseGenes))+
  ...
  geom_density_2d(color = "white", bins = 10)+
  theme(legend.position="none")+
  xlab("SNU1: Beta delta (aneuploid - euploid)")+
  ylab("-log2(p-value)")
# 5x4
# plot.MegaA_SNU1.pairedP.Diff_Highlight_MeanBeta0.pdf
# Figure 4C
...
# G-Profiler: 
g.WGL.SNU1.Toxic<- gost(
  subset(MegaA.Merge.SNU1.QN_meanDiff, Paired.Diff < -0.2)$Gene,
  organism = "hsapiens",
  ordered_query = FALSE,
  multi_query = FALSE,
  significant = TRUE,
  ...
  correction_method = c("g_SCS"),
  domain_scope = "custom",
  custom_bg = as.character(MegaA.Merge.SNU1.QN_meanDiff$Gene),
  numeric_ns = "",
  sources = NULL,
  as_short_link = FALSE
)

g.WGL.SNU1.Toxic_result<-g.WGL.SNU1.Toxic$result
g.WGL.SNU1.Toxic_result<-g.WGL.SNU1.Toxic_result[order(g.WGL.SNU1.Toxic_result$p_value),]
g.WGL.SNU1.Toxic_result
...
# "FBXO2"  "PLK3"   "SPI1"   "SPOP"   "TEAD3"  "UBE2H"  "UBE2N"  "ZNF146" "ZNF703" "ZNF784"


# random overlap for these datasets: 
Random10<- (3411*(341/3411)^4) # 0.3407002 genes would randomly overlap in top 10%
Random20<- (3411*(682/3411)^4) # 5.4 genes would randomly overlap in top 20%
...
ggplot(Merge.CRC, aes(x=AneuploidRank, y=CRC.Diff.paired, color= Gene == "UBE2H"))+
  geom_point()+
  geom_point(data = subset(Merge.CRC, Gene == "UBE2H"))+
  scale_color_manual(values=c("Black", "Red"))+ 
  theme_classic()+
...
Colorectal_Cell_Info<- subset(Cell_Info, OncotreeLineage=="Bowel") # 85 colorectal cancer 


Aneu_CRISPR_data<- merge(x=Aneuploidy_Scores, y=CRISPR_Gene, by.x="DepMap_ID", by.y="ModelID")
Cell_CRC<-  subset(Aneu_CRISPR_data, DepMap_ID %in% Colorectal_Cell_Info$ModelID)


CRISPR_DepMap_AneuScore_Colorectal<-data.frame(GeneID= character(), 
                                               Gene = character(), 
                                               Mean.Beta = numeric(),
                                               Aneu.CRISPR.Corr.P= numeric(),
                                               Aneu.CRISPR.Corr.Coef= numeric()
...
for (i in 6:length(Cell_CRC)){ #Depmap CRISPR data
  test1<- cor.test(Cell_CRC$Aneuploidy.score, Cell_CRC[,i])
  
  CRISPR_DepMap_AneuScore_Colorectal<- add_row(CRISPR_DepMap_AneuScore_Colorectal, data.frame(GeneID= colnames(Cell_CRC)[i],
                                                                                              Gene = sub("[..].*", "", colnames(Cell_CRC)[i] ),
                                                                                              Mean.Beta = mean(na.omit(Cell_CRC[,i])),
                                                                                              Aneu.CRISPR.Corr.P= test1$p.value, 
                                                                                              Aneu.CRISPR.Corr.Coef= test1$estimate ) ) 
} 
#length of Genes analyzed: 17386 Genes
CRISPR_DepMap_AneuScore_Colorectal<- CRISPR_DepMap_AneuScore_Colorectal[order(CRISPR_DepMap_AneuScore_Colorectal$Aneu.CRISPR.Corr.Coef),]
CRISPR_DepMap_AneuScore_Colorectal[1:20,]


## Now mark significance: 
# SigDiff is significant difference, being more toxic to aneuploid cells. P-value corrected for number of tests
# SigDiff_EuToxic is significant difference, being more toxic to EUPLOID cells. P-value corrected for number of tests
CRISPR_DepMap_AneuScore_Colorectal$SigDiff<- "FALSE"
for (i in 1:length(CRISPR_DepMap_AneuScore_Colorectal$GeneID)){
  if (CRISPR_DepMap_AneuScore_Colorectal$Aneu.CRISPR.Corr.P[i]< 0.05 
      & CRISPR_DepMap_AneuScore_Colorectal$Aneu.CRISPR.Corr.Coef[i]< 0) {
    CRISPR_DepMap_AneuScore_Colorectal$SigDiff[i]<- "TRUE"
  } 
}
subset(CRISPR_DepMap_AneuScore_Colorectal, SigDiff=="TRUE")$Gene
CRISPR_DepMap_AneuScore_Colorectal[1:20,]$Gene
...
HighlightTheseGenes<- c("TOX4" )
HighlightTheseGenes<- subset(CRC.MegaA.WGL.DepMap, Aneu.CRISPR.Corr.Coef< -0.1 & CRC.Diff.paired< -0.2 & Mean.Beta <0)$Gene
...
# G-Profiler: 
g.WGL.CRC.Toxic<- gost(
  subset(Merge.CRC, CRC.Diff.paired < -0.2)$Gene,
  organism = "hsapiens",
  ordered_query = FALSE,
  multi_query = FALSE,
  significant = TRUE,
  ...
  correction_method = c("g_SCS"),
  domain_scope = "custom",
  custom_bg = as.character(Merge.CRC$Gene),
  numeric_ns = "",
  sources = NULL,
  as_short_link = FALSE
)

g.WGL.CRC.Toxic_result<-g.WGL.CRC.Toxic$result
g.WGL.CRC.Toxic_result<-g.WGL.CRC.Toxic_result[order(g.WGL.CRC.Toxic_result$p_value),]
g.WGL.CRC.Toxic_result

setwd(ResultsFile)
# write.csv(g.WGL.CRC.Toxic_result[,1:13], "gProfiler.WGL.Ts2Toxic.Sheltzer.csv")
...
ggplot(Merge.St, aes(y=-log(Stomach.p.paired,2), x=Stomach.Diff.paired))+
  geom_point()+
  geom_point(data = subset(Merge.St, Stomach.p.paired< 0.05 & Stomach.Diff.paired< -0.2), color = "red")+
  scale_color_manual(values=c("Black", "Red"))+ 
  theme_classic()+
  #geom_vline(xintercept = 0)+
  geom_density_2d(color = "white", bins = 10)+
...
ggplot(Stomach.vs.nonStomach, aes(x= Stomach.Diff.paired, 
  ...
  theme_classic()
# 5x4
# plot.BetaDelta.Stomach.nonStomach_MeanBeta0.pdf 
...
# G-Profiler: 
g.WGL.STAD.Toxic<- gost(
  subset(Merge.St, Stomach.Diff.paired < -0.2)$Gene,
  organism = "hsapiens",
  ordered_query = FALSE,
  multi_query = FALSE,
  significant = TRUE,
  ...
  correction_method = c("g_SCS"),
  domain_scope = "custom",
  custom_bg = as.character(Merge.St$Gene),
  numeric_ns = "",
  sources = NULL,
  as_short_link = FALSE
)

g.WGL.STAD.Toxic_result<-g.WGL.STAD.Toxic$result
g.WGL.STAD.Toxic_result<-g.WGL.STAD.Toxic_result[order(g.WGL.STAD.Toxic_result$p_value),]
g.WGL.STAD.Toxic_result

setwd(ResultsFile)
# write.csv(g.WGL.STAD.Toxic_result[,1:13], "gProfiler.WGL.Ts2Toxic.Sheltzer.csv")



    #### Gain-dominant vs loss-dominant: SNU1 vs others (3 pairs) ####
# The only loss dominant cell lines we used are SNU1 clones 
# near tetraploid cells where subclones have losses 

# We have two loss-dominant clones: SNU1 Ts10.17.5 Ps2 and SNU1 Ts10.1.8.14 Ps5. 
# We have one Loss-only clone: SNU1 Ts9.19 
# and one evenly matched gain/loss clone: SNU1 Ts10.6q Ps8.12

# to identify loss dominant hits, we only analyzed the loss-dominant and loss-only clones. 
...
MegaA.Merge.Loss<- merge(Snu1.Cont1.C13.MegaA[,c(1,3)], Snu1.C12.MegaA[,c(1,3)], by=c("Gene", "Gene")) ### This one is wierd, low coverage leads to extreme negative beta scores
MegaA.Merge.Loss<- merge(MegaA.Merge.Loss, Snu1.C24.MegaA[,c(1,3)], by=c("Gene", "Gene"))
MegaA.Merge.Loss<- merge(MegaA.Merge.Loss, Snu1.C114.MegaA[,c(1,3)], by=c("Gene", "Gene"))


colnames(MegaA.Merge.Loss)<- c("Gene", "SNU1 WT", "SNU1 C12", "SNU1 C24", "SNU1 C114") 


# Remove all negative controls, except for three negative controls for heatmap plotting purposes. to show positive and negative controls. 
MegaA.Merge.Loss$Gene<- as.character(MegaA.Merge.Loss$Gene)
negControls<- MegaA.Merge.Loss[which(grepl("neg", MegaA.Merge.Loss$Gene)),] #list all negative controls. Not needed: neg controls already removed above
negControls<- negControls[-c(1,3,4),] #remove 3 negative controls for plot. 
MegaA.Merge.Loss <- subset(MegaA.Merge.Loss, ! Gene %in% negControls$Gene)   #remove rows with negative controls, except the three above


## Quantile normalize by batch and cell line: 
MegaA.Merge.Loss.QN<- MegaA.Merge.Loss

# Quantile normalize! 
MegaA.Merge.Loss.QN<- quantile_normalisation(MegaA.Merge.Loss.QN[,c(2:length(MegaA.Merge.Loss.QN))])
MegaA.Merge.Loss.QN<- as.data.frame(MegaA.Merge.Loss.QN)
MegaA.Merge.Loss.QN$Gene<- MegaA.Merge.Loss$Gene



## Now make p-value and average difference for SNU1

MegaA.Merge.Loss.QN_meanDiff<- MegaA.Merge.Loss.QN

# $meanDiff is mean difference in gene dropout between an aneuploid cell line and it's corresponding control cell 
MegaA.Merge.Loss.QN_meanDiff$Paired.Loss.Diff<- ( (MegaA.Merge.Loss.QN_meanDiff$`SNU1 C12` - MegaA.Merge.Loss.QN_meanDiff$`SNU1 WT`) + 
                                               (MegaA.Merge.Loss.QN_meanDiff$`SNU1 C24` - MegaA.Merge.Loss.QN_meanDiff$`SNU1 WT`) + 
                                               (MegaA.Merge.Loss.QN_meanDiff$`SNU1 C114` - MegaA.Merge.Loss.QN_meanDiff$`SNU1 WT`)
)/3

# Now add p-values to difference between all aneuploid and all euploid
MegaA.Merge.Loss.QN_meanDiff$p.paired.Loss<- NA
for (i in 1:length(MegaA.Merge.Loss.QN_meanDiff$Gene)){
  test<- t.test(as.numeric(MegaA.Merge.Loss.QN_meanDiff[i,c(2,3,4)]), 
                as.numeric(MegaA.Merge.Loss.QN_meanDiff[i,c(1,1,1)]), paired=TRUE) # t.test (aneuploid, euploid)
  MegaA.Merge.Loss.QN_meanDiff$p.paired.Loss[i]<-test$p.value
} #paired t-test

MegaA.Merge.Loss.QN_meanDiff$MeanBeta<- rowMeans(MegaA.Merge.Loss.QN_meanDiff[,1:4])



         ##### Gain-dominant #### 
# I want to compare chromosome-gain dominant specific hits to chromosome-loss specific hits 
# to differentiate between gain or loss dependencies 
# keep in mind that this is the limited druggable genome screen 
# keep in mind that the only chromosome loss cells we have are SNU1, a near tetraploid cell line. 
# Gain-dominant clones are mostly non-SNU1 cell lines, 
# and we are not including SNU1 Ts10.6q_Ps8.12 because it is near 50/50 gain/loss

Merge.nonSNU1<- Merge.ALL.QN2

Merge.nonSNU1$p.paired.Gain<- NA  #difference btween average an and average euploid beta score (aneuploid beta minus euploid beta)
Merge.nonSNU1$Paired.Gain.Diff<- NA 
Merge.nonSNU1<- Merge.nonSNU1 %>% relocate(Gene, .after = last_col()) #make sure Gene is at end of columns


for (i in 1:length(Merge.nonSNU1$Gene)){
  test<-t.test(as.numeric(Merge.nonSNU1[i, c(1, 3,3, 6,6,6, 10,10, 13,13, 16,16, 19,19, 27, 29, 32, 34,35, 37, 39)]), 
               as.numeric(Merge.nonSNU1[i, c(2, 4,5, 7,8,9, 11,12, 14,15, 17,18, 20,21, 18, 30, 31, 33,33, 36, 38)]), 
               alternative=c("two.sided"), paired=TRUE) 
  Merge.nonSNU1$p.paired.Gain[i]<- test$p.value
  Merge.nonSNU1$Paired.Gain.Diff[i]<- -1*test$estimate #make negative values more toxic to aneuploid
}
Merge.nonSNU1<- Merge.nonSNU1[order(Merge.nonSNU1$Paired.Gain.Diff),]
Merge.nonSNU1<- Merge.nonSNU1 %>% relocate("Gene") #more "Gene" gene names back to begining of columns


Loss.vs.Gain<- merge(MegaA.Merge.Loss.QN_meanDiff, Merge.nonSNU1[,c(1,41,42)], by.x = "Gene", by.y = "Gene")

# SigAllAneu2<- c("MAPKAPK2")
SigAllAneu2<- subset(Loss.vs.Gain, Paired.Loss.Diff< -0.2 & p.paired.Loss< 0.05 &
                       MeanBeta < 0 &
                       Paired.Gain.Diff > 0 # find gene that is not an gain-dominant dependency
                     )$Gene

ggplot(Loss.vs.Gain, aes(x= Paired.Loss.Diff, 
                                  y= Paired.Gain.Diff, 
                                  color= Gene %in% SigAllAneu2))+
  geom_point()+
  geom_hline(yintercept =0)+
  geom_vline(xintercept =0)+
  geom_vline(xintercept =-0.2, color="grey50", linetype = "dashed")+
  geom_point(data = subset(Loss.vs.Gain, Gene%in% SigAllAneu2))+
  scale_color_manual(values = c("black", "red"), 
                     name = "Loss Dominant")+
  #geom_point(data = subset(Loss.vs.Gain, Gene%in% c("UBE2H")), color="blue")+
  geom_density_2d(color = "white", bins = 10)+
  xlab("Difference in Beta score (Loss-dominant)")+
  ylab("Difference in Beta score (Gain-dominant)")+
  theme_classic()
# 5x4
# plot.BetaDelta.LossDominant.GainDominant_MeanBeta0.pdf


# Genes that chromosome-loss dominant cell lines (SNU1) are dependent upon that chromsome-gain dominant cell lines are not: 
subset(Loss.vs.Gain, Paired.Loss.Diff< -0.2 & p.paired.Loss< 0.05 & Paired.Gain.Diff >0 & MeanBeta < 0)$Gene
#  "EPHA1"    "IKZF3"    "IRF1"     "JDP2"     "KLHL20"   "LONP2"    "MAPKAPK2" "RFWD2"    "RPL23A"   "STK35"    "ZBTB18"  

# write.csv(Loss.vs.Gain, "GainDominant.vs.LossDominant.csv")


         ##### DepMap Loss-dominant scores ####

##  Find genes that are more toxic to loss-dominant aneuploid cells in DepMap data:

# Merge aneuploidy score and DepMap CRISPR data: 
Depmap.CRISPR.Loss<- merge(Arm_CN, CRISPR_Gene_all, by.x="X", by.y="ModelID")

# calculate the number of loss chromosomes minute the number of gain chromosomes:
Depmap.CRISPR.Loss$LossminGain<- -1*rowSums(Depmap.CRISPR.Loss[, 2:40])

#Now subset the data to only look at neutral and chromosome loss cells (if cell has more gains than losses, remove cell line)
Depmap.CRISPR.Loss<- subset(Depmap.CRISPR.Loss, LossminGain >-1)

list(Depmap.CRISPR.Loss$LossminGain)



### Now calculate and plot gene beta scores with aneuploidy score

CRISPR_DepMap_SumAneu<-data.frame(GeneID= character(), 
                                    Gene = character(), 
                                    Mean.Beta = numeric(),
                                    SampleNum = numeric(),
                                    
                                    Corr.LossminGain.p = numeric(),
                                    Corr.LossminGain.coef= numeric()
)

for (i in 41:length(Depmap.CRISPR.Loss)){
  x<- cor.test(Depmap.CRISPR.Loss[,i], Depmap.CRISPR.Loss$LossminGain, method = "pearson")
  
  CRISPR_DepMap_SumAneu<- rbind(CRISPR_DepMap_SumAneu, data.frame(GeneID= colnames(Depmap.CRISPR.Loss)[i],
                                                                      Gene = sub("[..].*", "", colnames(Depmap.CRISPR.Loss)[i] ),
                                                                      Mean.Beta = mean(na.omit(Depmap.CRISPR.Loss[,i])),
                                                                      SampleNum= length(Depmap.CRISPR.Loss[,i][!is.na(Depmap.CRISPR.Loss[,i])]), 
                                                                      
                                                                  Corr.LossminGain.p= x$p.value, 
                                                                  Corr.LossminGain.coef= x$estimate ) ) 
} 
CRISPR_DepMap_SumAneu$Gene<- sub("\\.\\..*", "", CRISPR_DepMap_SumAneu$GeneID) #Two periods \\.\\. followed by any character .* replace with nothing
CRISPR_DepMap_SumAneu$Gene<- sub("[.]", "-", CRISPR_DepMap_SumAneu$Gene) # replace single period with -. 


length(CRISPR_DepMap_SumAneu$Gene) # Length of Genes analyzed: 18444 Genes
CRISPR_DepMap_SumAneu<- CRISPR_DepMap_SumAneu[order(CRISPR_DepMap_SumAneu$Corr.LossminGain.coef),]
CRISPR_DepMap_SumAneu[1:20,]


CRISPR_DepMap_SumAneu$SigDiff<- "FALSE"
for (i in 1:length(CRISPR_DepMap_SumAneu$GeneID)){
  if (CRISPR_DepMap_SumAneu$Corr.LossminGain.p[i]< (0.05) && CRISPR_DepMap_SumAneu$Corr.LossminGain.coef[i]< -0.1){
    CRISPR_DepMap_SumAneu$SigDiff[i]<- "TRUE"
  }
}
subset(CRISPR_DepMap_SumAneu, SigDiff=="TRUE")

# plot DepMap correlation: gene dependency and sum of chromosome CN (positive values = loss dominant cells more sensitive than gain-dominant)
ggplot(CRISPR_DepMap_SumAneu, aes(y=-log(Corr.LossminGain.p,2), 
                                    x= Corr.LossminGain.coef, 
                                    color= SigDiff))+
  geom_point()+
  geom_point(data = subset(CRISPR_DepMap_SumAneu, SigDiff=="TRUE"))+
  theme_classic()+
  xlab("Correlation: gene dependency with number of loss minus gain arms \n<- values indicate loss-dominant aneuploid dependency")+
  ylab("-log2(P-value)")+
  scale_color_manual(values= c("Black", "Red"), name="P<0.05, Diff < -0.1")
# 5x4


# Merge DepMap and paired WGL Loss dependency: 
# find overlapping chromosome-loss specific dependencies: 
merge.LossDominant.Dependency<- merge(Loss.vs.Gain, CRISPR_DepMap_SumAneu, by.x="Gene", by.y="Gene")

x<-subset(merge.LossDominant.Dependency, 
          Paired.Loss.Diff< -0.2 & p.paired.Loss< 0.05 # identify genes that are significant dependencies in SNU1 loss-dominant aneuploidy
          & MeanBeta < 0 # identify genes that reduce growth when knocked out 
          & p.paired.Gain > 0.05 # identify genes that are NOT significant dependencies in gain-dominant aneuploidy
          & Corr.LossminGain.coef < -0.1 
          & Corr.LossminGain.p < 0.05)$Gene # identify genes that have a loss-dominant dependency in DepMap

ggplot(merge.LossDominant.Dependency, aes(x=Paired.Loss.Diff, # SNU1 dependency (loss dominant)
                                  y= Corr.LossminGain.coef))+ # depmap correlation with arm CN (negative correlation so that negative value indicate loss-dominant dependency)
  geom_point()+  
  geom_hline(yintercept =0)+
  geom_vline(xintercept =0)+
  geom_hline(yintercept = -0.1, color="grey50", linetype = "dashed")+
  geom_vline(xintercept =-0.2, color="grey50", linetype = "dashed")+
  geom_density_2d(color = "white", bins = 10)+
  geom_point(data = subset(merge.LossDominant.Dependency, Gene %in% x), color="red")+
  theme_classic()+
  xlab("Loss-dominant dependency (WGL)")+
  ylab("DepMap correlation: copy number sum & gene dependency\n <--loss-dominant dependency")
# 5x4 
#plot.LossDominantDependencies.DepMap.SNU1.pdf
subset(merge.LossDominant.Dependency, Gene %in% x)
# TRIM13: 
# UBC : 
subset(merge.LossDominant.Dependency, Gene %in% c("UBE2H", "FOSL1"))
#write.csv(merge.LossDominant.Dependency, "SNU1LossDominant.DepMapLossDominant.csv")
