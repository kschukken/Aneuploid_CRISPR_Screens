...
ggplot(Bokenkamp_protein, aes(x= (meanFCp0WT)))+
  geom_vline(xintercept=0, color="grey50")+
  geom_density(data = subset(Bokenkamp_protein, ! hgnc_symbol %in% c(RNA_Processing_rRNA, Ribosomal_Genes_noMT$Approved.symbol, MitoElectronChainnAssembly, MitoTranslationTranscription)), color= "black", size=1)+ 
  geom_density(data = subset(Bokenkamp_protein, hgnc_symbol %in% RNA_Processing_rRNA ), color="deepskyblue4", size=1) + #00BFC4
  geom_density(data = subset(Bokenkamp_protein, hgnc_symbol %in% Ribosomal_Genes_noMT$Approved.symbol ), color="#C77CFF", size=1) + #C77CFF
  geom_density(data = subset(Bokenkamp_protein, hgnc_symbol %in% MitoElectronChainnAssembly), color="#F8766D", size=1)+ #F8766D
  geom_density(data = subset(Bokenkamp_protein, hgnc_symbol %in% MitoTranslationTranscription), color="gold2", size=1)+   #7CAE00
  geom_density(data = subset(Bokenkamp_protein, hgnc_symbol %in% Proteosome_Genes$Approved.symbol ), color="chartreuse1", size=1) + #7CAE00
  geom_density(data = subset(Bokenkamp_protein, hgnc_symbol %in% RNA_Processing_Spliceosome ), color="deepskyblue1", size=1) + #00BFC4
  theme_classic()+
  xlim(-0.8,0.5)+
  theme(legend.position="none")+
  xlab(paste("log2 protein abundance fold change Aneu to WT"))+
  ylab(paste("density"))
# Boekenkamp.Protein.p0WT.Density.pdf
# Supplementary Figure S3H (left)
...
ggplot(Bokenkamp_protein, aes(x= (meanFCp50p0)))+
  geom_vline(xintercept=0, color="grey50")+
  geom_density(data = subset(Bokenkamp_protein, ! hgnc_symbol %in% c(RNA_Processing_rRNA, Ribosomal_Genes_noMT$Approved.symbol, MitoElectronChainnAssembly, MitoTranslationTranscription)), color= "black", size=1)+ 
  geom_density(data = subset(Bokenkamp_protein, hgnc_symbol %in% RNA_Processing_rRNA ), color="deepskyblue4", size=1) + #00BFC4
  geom_density(data = subset(Bokenkamp_protein, hgnc_symbol %in% Ribosomal_Genes_noMT$Approved.symbol ), color="#C77CFF", size=1) + #C77CFF
  ...
  xlab(paste("log2 protein abundance fold change P50 to P0"))+
  ylab(paste("density"))
# Boekenkamp.Protein.p50p0.Density.pdf
...
ggplot(Bokenkamp_protein, aes(x= (meanFCp50WT)))+
  geom_vline(xintercept=0, color="grey50")+
  geom_density(data = subset(Bokenkamp_protein, ! hgnc_symbol %in% c(RNA_Processing_rRNA, Ribosomal_Genes_noMT$Approved.symbol, MitoElectronChainnAssembly, MitoTranslationTranscription)), color= "black", size=1)+ 
  geom_density(data = subset(Bokenkamp_protein, hgnc_symbol %in% RNA_Processing_rRNA ), color="deepskyblue4", size=1) + #00BFC4
  geom_density(data = subset(Bokenkamp_protein, hgnc_symbol %in% Ribosomal_Genes_noMT$Approved.symbol ), color="#C77CFF", size=1) + #C77CFF
  ...
  xlab(paste("log2 protein abundance fold change Aneup50 to WT"))+
  ylab(paste("density"))
# Boekenkamp.Protein.p50WT.Density.pdf
# Supplementary Figure S3H
